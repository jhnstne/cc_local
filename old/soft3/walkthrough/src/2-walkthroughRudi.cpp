/*
  File:          walkthrough.cpp (from ugviewer.cpp)
  Author:        J.K. Johnstone 
  Created:	 15 November 2004
  Last Modified: 15 November 2004
  Purpose:       Test walkthrough using gluLookAt.
  Input:         
  History:
*/
...
V3fArr circleModel;     // circle for positioning of cylinders
...
void reshape(GLsizei w, GLsizei h)
{
  ...
  glLoadIdentity();
  gluPerspective (60.0, (GLfloat) w/(GLfloat) h, .1, 20.0); // orig from redbook/planet.c
...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case ' ':     pathFrame = (pathFrame+1)%100;  break;  // space
...
void displayOb ()
{
  ...
  //  fly around circle, gazing along path
  ...
  // fly around circle, gazing at center
  // gluLookAt(circlePath[pathFrame][0],circlePath[pathFrame][1],circlePath[pathFrame][2],
  //           0,0,0,
  //	       0,1,0);

  // gaze from the center, rotating gaze around entire circle
  // gluLookAt(-.5,0,0,
  //	       circlePath[pathFrame][0],circlePath[pathFrame][1],circlePath[pathFrame][2],
  //	       0,1,0);

  /*
    In general, the ingredients of a flythrough are
    - a flight path [this becomes the eye component of gluLookAt,
                     its tangent probably becomes the gaze component of gluLookAt]
    - an orientation along this flight path [expressing the orientation as a frame,
               two of the axes of the moving frame are used to define the gaze vector
	       and the up vector for gluLookAt]
    A flight path can be built by choosing points along the flight path using 
    a GUI manipulator of the flight's scene, such as Soda Hall.
    This GUI manipulator can also define orientations at the data points.

    Therefore, build a program that allows natural motion around a scene, similar to our
    present GUI, but also the ability to record the position and orientation of the 'eye'
    at any point. Use arrows to control motion forward/backward/left/right and two other
    keys for up/down.  This controls position alone.  Mouse control of orientation is
    probably most natural.
    We will build this interface into a new version of ugviewer.
   */
  ...
  //  move the camera along a circular path running around the inside of the track of cyls

  //  draw a circle of several cylinders (cubes are easier) with a sphere in the middle
  glDisable(GL_LIGHTING);
  glColor3fv (Black);
  glutWireSphere(.1,20,20);
  for (i=0; i<10; i++)
    {
      glColor3fv (material[i]+4);
      glPushMatrix();
      glTranslatef (circleModel[i][0], circleModel[i][1], circleModel[i][2]);
      glScalef (1,10,1);
      glutWireCube (.1);
      glPopMatrix();
    }
