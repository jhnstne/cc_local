...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case 'n':     transzob -= .01;                break; // backward
  case 'u':     transzob += .01;                break; // forward
  case 'h':     transxob += .01;                break; // left
  case 'k':     transxob -= .01;                break; // right
  case 'q':     transyob -= .01;                break; // up
  case 'a':     transyob += .01;                break; // down
  ...
  case ' ':     // record present position
                keyframe[nKey][0] = -transxob; // correct
                keyframe[nKey][1] =  transzob; // correct
                keyframe[nKey][2] = -transyob; // correct
		outfile << keyframe[nKey][0] << " " 
		        << keyframe[nKey][1] << " "
		        << keyframe[nKey][2] << endl;
		nKey++;
		if (nKey >= 1000) 
...
void displayOb ()
{
  ...
  glRotatef (rotxob, 1.0, 0.0, 0.0);  // rotate after translation?
...
int main (int argc, char **argv)
{
  ...
  */
  ...
  if (KEYFILE)
    {
      for (i=0; i<nKey; i++)
	outfile << keyframe[i][0] << " " << keyframe[i][1] << " " << keyframe[i][2] << endl;
    }
