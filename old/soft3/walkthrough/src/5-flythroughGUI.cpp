/*
  File:          flythroughGUI.cpp (from flythroughPos.cpp)
                 more natural GUI: move in present direction using gluLookAt;
		 adjust gaze direction and always move in gaze direction,
		 rather than absolute or relative coordinate axis motions.
...
V3f presentPt(0,0,0);   // location in world
V3f origGaze (0,1,0);   // original gaze direction, looking at floor from normal pt of view
V3f presentGaze(0,1,0); // unit vector in current direction of motion
  ...
  if (WINDOWS)
   {
	...
	{
	  presentGaze[0] += .05*(x-oldx); 
	  float x = presentGaze[0], z = presentGaze[2];
	  presentGaze[1] = sqrt(1-x*x-z*z);
	}
  ...
  else
   {
	...
	{ 
	  rotzob += .5*(x-oldx); if (rotzob > 360.0) rotzob -= 360.0; // flythrough GUI
	  presentGaze = origGaze;
	  rotAboutX (presentGaze, rotxob);
	  rotAboutY (presentGaze, rotyob);
	  rotAboutZ (presentGaze, rotzob);
	  //	  presentGaze[0] += .05*(x-oldx);
	  //	  float x = presentGaze[0], z = presentGaze[2];
	  //	  presentGaze[1] = sqrt(1-x*x-z*z);
	}
...
{
  float xcoord = presentGaze[0], z = presentGaze[2];
  switch (key) {
  ...
  case 'n':     break; // backward
  case 'u':     break; // forward
  case 'h':     break; // left
  case 'k':     break; // right
  case 'q':     break; // up
  case 'a':     break; // down
  case 'x':     presentGaze[0] += .05; 
                xcoord = presentGaze[0]; 
		presentGaze[1] = sqrt(1-xcoord*xcoord-z*z); break; // rotate about x
  case 'X':     presentGaze[0] -= .05; 
                xcoord = presentGaze[0]; 
		presentGaze[1] = sqrt(1-xcoord*xcoord-z*z); break;
//  case 'y':     presentGaze[1] += .05; presentGaze.normalize(); break; // rotate about y
//  case 'Y':     presentGaze[1] -= .05; presentGaze.normalize(); break; 
//  case 'z':     presentGaze[2] += .05; presentGaze.normalize(); break; // rotate about z
//  case 'Z':     presentGaze[2] -= .05; presentGaze.normalize(); break; 
  case 'b':     break;
  case ' ':     for (int i=0; i<3; i++)
                  presentPt[i] += .01 * presentGaze[i]; break;
    /*
...
void displayOb ()
{
  ...
  glMatrixMode (GL_MODELVIEW);
  glLoadIdentity();
  gluLookAt(presentPt[0], 
	    presentPt[1], 
	    presentPt[2],
	    presentPt[0] + presentGaze[0], 
	    presentPt[1] + presentGaze[1], 
	    presentPt[2] + presentGaze[2],
	    0,0,1);
  ...
  if (DRAWCAMERA)
    {
      V3f redPt(0,0,-.1); float red[4] = {1,0,0,1};  // reference pt travelling w. us
