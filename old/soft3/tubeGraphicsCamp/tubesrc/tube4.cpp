/*
  ...
  Purpose:	 Geometric modeling of a tube.
  		 A project for UAB Computer Science Camp.
		 Version for day 4.
  History:	 6/7/01: Corrected M_PI
*/
...
#define DENSITY		10
#ifndef M_PI
#define M_PI		3.141593  // M_PI should be in math.h, but this is Microsoft...
#endif
...
{
 int i;
 V3f ab,ac; for (i=0; i<3; i++) { ab[i] = b[i]-a[i]; ac[i] = c[i]-a[i]; }
 V3f norm;  norm.cross (ab,ac); norm.normalize();	// need unit vector!
...
{
  // use computeTriNormal to set the normal
  // you can try two methods
  // (1) GL_TRIANGLES, drawing 'two triangles at a time'
  // (2) GL_TRIANGLE_STRIP, more subtle handoff between triangles
}
...
int main (int argc, char **argv)
{
  ...
  float maxScale = mymax (scale[0], mymax (scale[1], scale[2]));
  for (i=0; i<nPt; i++)
    for (j=0; j<3; j++)
