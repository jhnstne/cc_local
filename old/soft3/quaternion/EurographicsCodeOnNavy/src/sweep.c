...
#include "/usr/people/jj/cbin/bezsurfWithNormals.c"
...
*/
	...
	if (KEYFRAMES)
		sweep_wid = initialize_3d_window("Input",25,400,200);
	else if (SURF)
		   ...
		   tp_ratbez 	*surf)
{
	...
	if (KEYFRAMES || INTERMEDIATE) {      /* draw keyframes */
	   linewidth(5);
/*	   RGBcolor(255,0,0); */
	   RGBcolor(0,0,0);
	   for (i=0;i<m;i++) {
	...
	if (INTERMEDIATE) {		/* draw intermediate frames */
	   RGBcolor(0,0,255);
 	   for (i=0;i<m-1;i++) {
