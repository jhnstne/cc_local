...
\begin{abstract}
A rational quaternion spline is desirable for the representation of orientation
of a moving object when we want to design free motions through a scene, such
as in flythroughs.
A natural and common approach to motion is as follows.
Landmark keyframes can be established in the scene and the motion built through these
keys, but the resulting motion may not be a good one: it may intersect the 
scene in between the keyframes.
To test for interference, the swept envelope of the object should be calculated and
intersected with the scene.
It is here that a rational representation for the motion has especial benefit:
the envelope can be represented as a rational Bezier surface, which allows
efficient intersection algorithms to be applied.

(This approach is conducive both to an interactive design of an optimal motion and to
the design of a motion from a high level cell decomposition approach to motion planning,
where a channel through the scene is defined, and keyframes are placed in
each cell of the channel to determine the exact motion.  [But what is the orientation
of each cell's representative?])

Rational flythrough as 671 project: encompasses scene definition (building), 
object definition (animated object), curve definition (path),
orientation definition (quaternion spline), maps to and from the sphere,
rational curves and projective space (quaternion spline), 
derivatives (smooth connection between components of quaternion spline), 
end conditions, degree elevation (quaternion spline),
surface definition (swept envelope): could use ruled surface for first iteration,
intersection (collision detection), motion planning, potentially visibility,
OpenGL.
add an independent advanced OpenGL project on buffers 
such as antialiasing (accumulation buffer),
and texture maps, which is necessary insight for GPU simulation of conventional algorithm.
A separate course is needed for a treatment of Cg, Brook, streams and GPU programming,
illustrating vertex and fragment shaders and their use in general programming.
Perhaps next year's 672, with an interim seminar or two.

Short term application and testbed for orientation control: building flythrough.
I need to gather orientations from keylocal in csb5.macro.ug.
(Think of camera flydown from above fenced jail corridor in Smallville.
Think of pivoting head up from bending down to floor to pick up a ring
to looking for an approaching marauder.)
Read and mention camera control papers.
Mention game design applications.
Compare to naive Frenet frame solution and slerping solution.
