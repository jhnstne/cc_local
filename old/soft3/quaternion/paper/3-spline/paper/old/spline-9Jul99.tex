...
Keywords: quaternion, quaternion spline, orientation, animation, 
...
is a unit quaternion, a point on the unit sphere \Sn{3}\ in 4-space.
...
\begin{defn2}
\Sn{3}\ is the unit sphere in 4-space, $x_1^2 + x_2^2 + x_3^2 + x_4^2 - 1 = 0$.
(The superscript 3 refers to the dimension of the manifold.)
A {\bf unit quaternion} is a point on \Sn{3}, interpreted as an orientation.
...
An interpolating curve on \Sn{3},
...
The construction of a good rational map to \Sn{3}\ is considerably
more challenging.
...
They work on the SO(3) manifold, rather than \Sn{3}\ manifold.
...
It turns out that a parameterization of \Sn{3}\ is not a great map
...
That is, \Sn{3}\ is a double covering of SO(3).
...
\end{defn2}
%
We shall use the map $M$ as our map to \Sn{3}.
...
of rational maps from $\Re^4$ to \Sn{3}\ and quaternions.
...
the inverse of the map to \Sn{3}\ is needed (step 1b).
...
\end{theorem}
\prf 
The proof is simplified by working in projective space,
where the map $M$ becomes
\begin{equation}
\label{eq:proj}
...
Notice the beautiful simplicity of the defining point.
...
\bibitem[Preparata and Shamos 85]{shamos85}
