...
/******************************************************************************
	Map tangent on S3 to tangent in Euclidean space, at data point.
	Lemma 9.3
******************************************************************************/
    
void imageTangOnS3 (V4r pt, V4r tang, V4r imageTang)
{
  float y,y2;
  int i;
  y2 = 2. - 2.*pt[0];
  y  = sqrt(y2);
  for (i=0; i<3; i++)
    imageTang[i] = (tang[i+1]*y2 + pt[i+1]*tang[0]) / y;
  imageTang[3] = (tang[0]*y2 + (1.-pt[0])*tang[0]) / y;
}
...
static void MapInput (int n, V4r *pt, V4r begTang, int SPflag, V4r *imagept, 
		      V4r imageBegTang)
{
  int i,j;
  V4r definingPt;
  ...
  if (SPflag)
   {
    ...
    /* for now, just so tangent is well defined */ Copy4r (imageBegTang, begTang);
    return;
   }
  for (i=0; i<n; i++)
   {				/* (p2,p3,p4,1-p1) */
  ...
  /* compute image of beginning tangent */
  if (IMAGEONS3)    imageTangOnS3 (pt[0], begTang, imageBegTang);
  else
   {
    /* wrong */
    alpha = imagept[i][0] / definingPt[0];
    for (j=0; j<3; j++) imageBegTang[j] = alpha * begTang[j+1];
    imageBegTang[3] = -1. * alpha * begTang[0];
   }
...
static void DesignImageCurve (int n, V4r *imagept, V4r begTang, int SPflag, 
			      Bez4d *imageBez, 
...
{	
  ...
  if (ExactEqual4r (begTang, zerovec))
    FitCubicBez_4d (n, imagept, imageBez);
  else
    FitCubicBezOneEndTangent_4d (n, imagept, 1, begTang, imageBez);
...
/******************************************************************************
	begTang is input to the S3spline: 
...
void S3spline (int n, V4r *pt, V4r begTangOnS3,
	       ...
	       V4r **S3curveDisplay)
{
  ...
  V4r   imageBegTang;		/* beginning tangent in Euclidean space */

  *perturbpt = (V4r *) malloc (n * sizeof(V4r));
  PerturbInput (n, pt, begTangOnS3, PERTURB, *perturbpt, perturbBegTangOnS3, 
  		RestoringRotation);
  *imagept  = (V4r *) malloc (n * sizeof(V4r));
  MapInput (n, *perturbpt, perturbBegTangOnS3, SPflag, *imagept, imageBegTang);
  DesignImageCurve (n, *imagept, imageBegTang, SPflag, imageBez, 
  		    imageCurveDisplay, ptsPerBezSegment);
...
int main(int argc, char **argv)
{
  ...
  for (i=0; i<nSub; i++)  subpt[i] = (V4r *) malloc (m[i] * sizeof(V4r));
  for (i=0; i<nSub; i++)
    for (j=0; j<m[i]; j++)
  ...
  for (i=0; i<nSub; i++)
   {
    S3spline (m[i], subpt[i], begTang, PERTURB, &(perturbpt[i]), 0,
    ...
    S3spline (m[i], subpt[i], begTangSP, PERTURB, &(perturbpt[i]), 1,
    ...
    BegTangRat_4d (&(S3curve[i]), beginTang[i]);
printf("Beginning tangent of subcurve %i is (%f,%f,%f,%f)\n", i,
	beginTang[i][0], beginTang[i][1], beginTang[i][2], beginTang[i][3]);    
    /* set begTang for next subcurve to end tangent of S3curve[i] */
    EndTangRat_4d (&(S3curve[i]), endTang[i]);
printf("End tangent of subcurve %i is (%f,%f,%f,%f)\n", i,
	endTang[i][0], endTang[i][1], endTang[i][2], endTang[i][3]);
    Copy4r (begTang, endTang[i]);
    EndTangRat_4d (&(perturbS3curve[i]), perturbEndTang[i]);
    EndTang_4d (&(imageBez[i]), imageEndTang[i]);

    printf("Perturbed end tangent on S3: (%f,%f,%f,%f)\n", 
    	perturbEndTang[i][0], perturbEndTang[i][1], perturbEndTang[i][2], 
	perturbEndTang[i][3]);
    printf("Image of this end tangent: (%f,%f,%f,%f)\n", 
    	imageEndTang[i][0], imageEndTang[i][1], imageEndTang[i][2], 
	imageEndTang[i][3]);
    imageTangOnS3 (perturbpt[i][m[i]-1], perturbEndTang[i], foo);
    printf("Anticipated image of this tangent: (%f,%f,%f,%f)\n",
    	    foo[0], foo[1], foo[2], foo[3]);
/*    alpha = imageEndTang[i][0] / perturbEndTang[i][1];
    printf("alpha: %f\n", alpha);
    printf("theoretical image tangent: (%f,%f,%f,%f)\n", 
    		alpha * perturbEndTang[i][1], alpha * perturbEndTang[i][2], 
		alpha * perturbEndTang[i][3], -1. * alpha * perturbEndTang[i][0]);
*/
