/*
  File:          orientationPlan.cpp
  Author:        J.K. Johnstone 
  Created:	 28 February 2004
  Last Modified: 28 February 2004
  Purpose:       Manipulate the orientation of an object, outputting the rotation angles
                 rotx(yz)ob interactively, with the aim of defining the quaternion
		 constraints for an orientation obstacle (orientations at boundary
		 between legal and illegal orientations).

		 At present, orientation is output in Euler angles,
		 but this should be changed to axis/angle or quaternion eventually.
		 We should also allow certain orientations to be stored, to accumulate
		 samples for the orientation obstacle.
		 Question: are there one or 2 degrees of freedom to these samples?
		 That is, will they define a curve or surface?  I think a curve, since
		 there appears to be 1dof as we define the boundary of illegal orientations.
  Sequence:	 3rd in a sequence (interpolate, with GUI of surfinterpolate; revosurf; orientationPlan)
...
Scene                   scene;
ofstream                outfile;        // repository for (boundary) illegal orientations
...
*/
  ...
  zoomob = .1;
  if (BEZINPUT) { rotxob = 270; zoomob = 1.8; } // see surfinterpolate.cpp
}
...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case 'i':     cout << "Saving this illegal orientation:" 
                     << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
                outfile << rotxob << " " << rotyob << " " << rotzob << endl;
		                                break;
  case 'x':     rotxob += 1; if (rotxob >= 360) rotxob -= 360; 
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  case 'y':     rotyob += 1; if (rotyob >= 360) rotyob -= 360;
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  case 'z':     rotzob += 1; if (rotzob >= 360) rotzob -= 360;
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  case 'X':     rotxob -= 1; if (rotxob < 0) rotxob += 360; 
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  case 'Y':     rotyob -= 1; if (rotyob < 0) rotyob += 360;
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  case 'Z':     rotzob -= 1; if (rotzob < 0) rotzob += 360;
                cout << "(" << rotxob << "," << rotyob << "," << rotzob << ")" << endl; 
		                                break;
  default:      break;
  }
...
void displayOb ()
{
  ...
  if (DRAWGRID)       // draw reference grid (floor of scene) in y=0 plane
    {                 // not affected by orientation changes
...
int main (int argc, char **argv)
{
  ...
  outfile.open ("illegal.dat"); // output (extremal) illegal orientations here
  outfile << "Illegal orientations from " << argv[argc-1] <<" in xyz Euler angles."<<endl;
