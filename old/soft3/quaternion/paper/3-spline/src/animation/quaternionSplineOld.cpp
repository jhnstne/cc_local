...
static GLboolean DRAWSPLINE=0;          // draw quaternion spline?
...
int main (int argc, char **argv)
{
  ...
  if (RANDOMINPUT)                                          // input
    qspline.randomData (nRandom, maxAngle, 1);
  else
   {
    ifstream infile;  infile.open(argv[argc-1]);
    qspline.readQuat (infile, AXISANGLEINPUT ? 1 : 0, maxAngle, 1);
    infile.close();
   }
  cout << "Fitting ..." << endl;
  qspline.fit (maxAngle, 0);
