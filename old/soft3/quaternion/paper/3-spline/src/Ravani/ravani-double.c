...
typedef double Matrix3[3][3];	/* 3x3 matrix type */
...
int	  nSample;
V4d      *ptSample;		/* point samples from Lie curve on S3 */
...
static void skew2vec (Matrix3 mat, V3d vec)
{
  vec[0] = mat[2][1];
  vec[1] = mat[0][2];
  vec[2] = mat[1][0];
...
static void vec2skew (const V3d vec, Matrix3 mat)
{
...
static void cross (V3d u, V3d v, V3d w)
{
 w[0] = u[1]*v[2] - u[2]*v[1];
 w[1] = u[2]*v[0] - u[0]*v[2];
 w[2] = u[0]*v[1] - u[1]*v[0];
...
void Lieexp (const V3d skew, Matrix3 rot)
{
  int i,j;
  double norm, foo, bar, m[3][3], mm[3][3];
...
void Lielog (const Matrix3 rot, V3d skew)
{
  int   i,j;
  double phi, trace, c;
  double mat[3][3];
...
void RotMatrix2Qion (Matrix3 m, V4d q)
{
  double w2,x2,y2;
...
int main(int argc, char **argv)
{
  ...
  int      ArgsParsed=0;
  Matrix3  Rt, RR;
  V3d     *r;		/* skew-symmetric matrices assoc with data pts */
  Matrix3 *A;
  double   norm, foo, bar;
  Matrix3  m,mm;
  V3d      s,t,u;
  V3d     *a,*b,*c;
  double   cosnorm, sinnorm, dot, norm2, foo1, foo2, foo3, foo4;
  V3d      st, su, sst, tst, ssu;
  ...
  r    = (V3d *) malloc (n * sizeof(V3r));
  A    = (Matrix3 *) malloc (n * sizeof(Matrix3));
  a    = (V3d *) malloc (n * sizeof(V3r));
  b    = (V3d *) malloc (n * sizeof(V3d));
  c    = (V3d *) malloc (n * sizeof(V3d));
  nSample = 2*nProbes;
  ptSample = (V4d *) malloc (nSample * sizeof(V4d));
...
printf("tauvec: (%lf,%lf,%lf)\n", tauvec[0], tauvec[1], tauvec[2]);
  ...
  printf("Covariant acceleration of Lie curve: %lf\n", covAccel);
