/*
  ...
  Created:	 31 May 2006, from surfinterpolate.cpp 3/14/03 version (extension to IRIT data)
  Last Modified: 31 May 2006
  Purpose:       Construction of smooth Bezier models, from
                 (a) cubic Bezier meshes, (b) Bspline meshes in IRIT format, or 
		 (c) data points in tensor product format.
  Sequence:	 1st in a sequence (surfbuild, tangSurf, bisilh)
  Input:         Bezier mesh, Bspline mesh in IRIT format, or rectangular mesh of data points.
  Output: 	 Smooth Bezier model
  ...
  Future:        Add rational Bezier/Bspline input.
...
#include "basic/Miscellany.h"           // countIRITmodel
...
#include "surf/BsplineSurf.h"           // readIRIT
...
static void usage()
 {
  ...
  cout << "\t file.cpt3 (tensor product Bezier control mesh) |" << endl;
  cout << "\t file.pts3 (rectangular array of data points) |" << endl;
  cout << "\t file.itd  (tensor product Bspline control mesh in IRIT format)" << endl;
 }
...
static GLboolean ROTATEMOD=0;		// rotate models?
static GLboolean PANMOD=0; 		// rotate object back and forth for 3d effect?
...
static GLboolean UNIFIEDCTRLMESH=0;	// input data is unified control mesh? 
                                        // (not just a collection of separate bicubic patches)
...
Array<BezierSurf3f> 	model;	// smooth Bezier models
...
int			modelWin;	// window identifier 
...
void visibility (int status)
{
  if (status != GLUT_VISIBLE)
    glutIdleFunc (NULL);
  else if (ROTATEMOD) glutIdleFunc (RotateOb);
  else if (PANMOD)    glutIdleFunc (PanOb);
}
...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case 'r':     ROTATEMOD = !ROTATEMOD;			// rotate
 	     	if (ROTATEMOD) 
		     glutIdleFunc (RotateOb); 
		else glutIdleFunc (NULL); 	break;
  case 'p':	PANMOD = !PANMOD;				// pan
		if (PANMOD) 
		     glutIdleFunc (PanOb); 
		else glutIdleFunc (NULL); 	break;
...
{ 
  ...
  if (suffix == "itd")                            // Elber's IRIT format
    {
      int mark;  mark = infile.tellg();
      int nModel = countIRITmodel (infile); // how many surfaces (ignoring nesting)?  
      cout << nModel << " models" << endl;
      Array<BsplineSurf3f> bsplmodel(nModel); 
      infile.clear();  infile.seekg(mark);
      readIRIT (infile, bsplmodel); // read the IRIT file to define B-spline surfaces  
      for (i=0; i<bsplmodel.getn(); i++) cout << "Model i:" << bsplmodel[i] << endl;
      infile.close();
      model.allocate(nModel);
      for (i=0; i<nModel; i++)
	{
	  model[i].translate (bsplmodel[i]);
	  cout << "Model " << i << ":" << endl << model[i] << endl;
	}
      cout << "Done translating B-spline surfaces to Bezier" << endl;
    } 
  else
    {
      read (infile, Pt);  scaleToUnitCube (Pt);         // Bezier mesh or data points
	  ...
	  if (CTRLMESH)	// each Pt[i] is a bicubic patch
	    {
	      ...
	      model[i].create (3, 3, 3, 1, 1, Pt[i], knot, knot);  // define one bicubic patch
	    }
  ...
  for (i=0; i<model.getn(); i++)  model[i].prepareDisplay (nPtsPerSegment);
  cout << "Done preparing display" << endl;
  uFirstKnot = model[0].getKnotu(0);	
  vFirstKnot = model[0].getKnotv(0);
  uLastKnot  = model[0].getKnotu (model[0].getnKnotu()-1);
  vLastKnot  = model[0].getKnotv (model[0].getnKnotv()-1);
  ...
  modelWin = glutCreateWindow (titlebar);
  gfxinit();
