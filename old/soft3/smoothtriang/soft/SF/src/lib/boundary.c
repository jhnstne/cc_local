			  ...
			  Scalar jensen)
{
  ...
  MinMaxBoundary(s0,S1,T0, T3,S2,s3,T1,T2,a0,a3,
		 EdgeTangent(s0,A),EdgeTangent(s3,A),R0,R3,1);
#endif
}
...
/*
 *  Function:  PlanarBoundary
 *	Sets tangents at either end of a single border curve.
 */
		     ...
		     int scaledNielson)
{
	...
	t0 = VNormalize(VVCross(VVCross(vn0, PPDiff(p1, p0)), vn0));
	t1 = VNormalize(VVCross(VVCross(vn1, PPDiff(p1, p0)), vn1));

	if ( scaledNielson ) {
		t0 = SVMult(PPDist(p0, p1), t0);
		t1 = SVMult(PPDist(p0, p1), t1);
	}

	*r0 = PVAdd(p0, SVMult(1./3., t0));
	*r1 = PVAdd(p1, SVMult(-1./3., t1));
}
