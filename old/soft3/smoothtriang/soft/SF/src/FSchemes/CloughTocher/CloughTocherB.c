...
/*
 *----------------------------------------------------------------------
 *  File:  CloughTocherB.c
 *----------------------------------------------------------------------
 */

/*
** Authors: Tony DeRose and Steve Mann
** Last Modified: 04/13/89 at 10:37:59
** Purpose: An implementation of Clough-Toucher for
**          constructing C1 cubic surface.  This version splits
**	before computing the crossboundary derivative.  This
**	gives an identical result as splitting after computing
**	the crossboundary point (although splitting after is
**	more efficient).
**
*/
#include <stdio.h>
...
void ProcessFace( Face* f )
{
    ...
    ForeachFaceVertex(f, v) {
	    if ( i >= 3 ) {
		    fprintf(stderr,"CloughTocherB: ProcessFace(%s): too many vertices\n",ReturnName(f));
		    exit(1);
	    }
	    vr[i++] = v;
    } EndForeach;
...
main(int argc, char **argv)
{
	...
	AddGeometry(World, m);
        ...
        if ( !(ReturnUDGeoFlags(m) & G_NORMAL) ) {
                fprintf(stderr,"CloughTocherB: normals not given.  Exiting.\n");                exit(1);
        } 
        ForeachMeshFace(m,f){
        ...
        if (verbose){
                ...
                fprintf(stderr,"CloughTocherB: finished.  ");
...
void SetShapeParam(char* argv[])
{
   ShapeParameter = atof(argv[0]);
}

char* Banner = "CloughTocherB";
char* UsageString = "CloughTocherB [option] < mesh";
Option Options[] = {
/* Name,        Routine,        #args,  Help String */
   "h",         Usage,          0,      ":      Print available options.",
   "s",		SetShapeParam,	1,	"s:     Set shape parameter [0.25]",
};
