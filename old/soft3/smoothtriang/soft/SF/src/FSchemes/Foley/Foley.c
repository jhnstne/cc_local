/*
 * Copyright (c) 1996, Computer Graphics Labratory, University of Waterloo
		...
		Scalar *k0, Scalar *k1, Scalar *h0, Scalar *h1)
{
    ...
    F0 = PPDiff (	PatchGetPoint (*patch, 3, 0, 0),
			PatchGetPoint (*patch, 2, 0, 1));
    F0 = SVMult (3.0, F0);
    F1 = PPDiff (	PatchGetPoint (*patch, 0, 3, 0),
			PatchGetPoint (*patch, 0, 2, 1));
    F1 = SVMult (3.0, F1);

    C0 = PPDiff (	PatchGetPoint (*npatch, 2, 0, 1),
			PatchGetPoint (*patch, 2, 0, 1));
    C1 = PPDiff (	PatchGetPoint (*npatch, 0, 2, 1),
			PatchGetPoint (*patch, 0, 2, 1));
    C0 = VNormalize (C0);
    C1 = VNormalize (C1);

    H0 = PPDiff (	PatchGetPoint (*patch, 2, 1, 0),
			PatchGetPoint (*patch, 3, 0, 0));
    H0 = SVMult (3.0, H0);

    H1 = PPDiff (	PatchGetPoint (*patch, 0, 3, 0),
			PatchGetPoint (*patch, 1, 2, 0));
    H1 = SVMult (3.0, H1);

    frame0 = FCreate ("Gethk frame 0", PatchGetPoint (*patch, 3, 0, 0),
		    C0, H0, VVCross (C0, H0));
    frame1 = FCreate ("Gethk frame 1", PatchGetPoint (*patch, 0, 3, 0),
		    C1, H1, VVCross (C1, H1));
...
BOOLEAN GetInterior (Face* face, Vertex* v0, Vertex* v1, Vertex* v2, Point* p)
{
	...
	Scalar	a, b, c, d, e, h0, h1, k0, k1, c0, c1, cx, cy, x;
	Vector	vecC0, vecC1;
	static int	first = 1;
	...
	vecC0 = PPDiff (	PatchGetPoint (npatch, 2, 0, 1),
				PatchGetPoint (patch, 2, 0, 1));
	vecC1 = PPDiff (	PatchGetPoint (npatch, 0, 2, 1),
				PatchGetPoint (patch, 0, 2, 1));
	...
	b = 2.0;
	c = c1;
	d = -2  * h0 * (b120-b210) - h1  * (b210-b300) + k0 * c0 / 3.0
	    + k1 * c0 / 3.0 - k0 * c1 / 3.0 + 3 * b210 - b201;
	e = - h0  * (b030-b120) - 2 * h1 * (b120-b210) + k0 * c1 / 3.0
	    - k1 * c0 / 3.0 + k1 * c1 / 3.0 + 3 * b120 - b021;

#define MATTHEWS_EPS	0.001
	if (fabs(a-c) < MATTHEWS_EPS) {
		...
		fprintf (stderr, "a(%lf), c(%lf), d(%lf), e(%lf) %lf and %lf\n",a,c,d,e,fabs(a-c),fabs(d-e));
		exit(1);
	    } /* if  */
	} else {
	    x = (d-e)/(a-c);
	    b111z = (e - c*x) / b;
	    /* b111z = e / b; */
	    /* fprintf (stderr, "a(%lf), c(%lf), d(%lf), e(%lf), x(%lf)\n",a,c,d,e,x); */
	    /* fprintf (stderr, "fsbs(a-c) != 0\n"); */
	    if ( fabs(b111z - ( (d-a*x)/b)) > 1e-6 ) {
		    fprintf(stderr,"Awk!\n");
	    }
	} /* if  */
	...
	if ( first ) {
	    fprintf(stderr,"Using Clough-Tocher centre points\n");
	    first=0;
	}

#if 0
	/* - take average of points (0,1,2), (1,0,2), (0,2,1), and (2,0,1)
	 */
	ptA = PPac( 
		    PPac (	PatchGetPoint(npatch,0,1,2), 
				PatchGetPoint(npatch,1,0,2), 1.0/2.0),
		    PPac (	PatchGetPoint(npatch,0,2,1), 
				PatchGetPoint(npatch,2,0,1), 1.0/2.0),
		    1.0/2.0);

	ptB = PPac(	
		    PPac (	PatchGetPoint(patch,0,1,2), 
				PatchGetPoint(patch,1,0,2), 1.0/2.0),
		    PPac (	PatchGetPoint(patch,0,2,1), 
				PatchGetPoint(patch,2,0,1), 1.0/2.0),
		    1.0/2.0);
#elif 0
	/* - take average of points (0,1,2) and (1,0,2)
	 *   i.e. two point farthest from boundary
	 */
	ptA = PPac( PatchGetPoint(npatch,0,1,2), 
		    PatchGetPoint(npatch,1,0,2), 1.0/2.0);
	ptB = PPac(	PatchGetPoint(patch,0,1,2), 
			PatchGetPoint(patch,1,0,2), 1.0/2.0);
#else
	...
	tempv1 = FV( StdFrame(SpaceOf(p1)), 2 );
	*p = LineIntersectPlane(p1,tempv1,point,n);
