...
char **argv;
{
  ...
  if ( !(ReturnUDGeoFlags(m) & G_NORMAL) ) {
    fprintf(stderr,"TriGregory: normals not given.  Exiting.\n");
    exit(1);
  } 
  ...
  if (verbose){
    ...
    fprintf(stderr,"TriGregory: finished.  ");
 ...
 *                                                                      
 */
GregoryPatch AdjustPatch(patch)
Patch patch;
{
  ...
  for( I=0; I<3; I++){
    if ( jensen != 0. ){
      ...
      ChiyokuraKimura( V[I], C[I][J], Q[I][K],  Q[J][K], C[J][I], V[J],
		      &(gp.pt[K][0]), &(gp.pt[K][1]), 
		       V[I], V[I], 0);/* note: V[I] are dummies */
    }
  }
  return gp;
}
...
char* Banner = "TriGregory";
char* UsageString = "TriGregory [option] < mesh ";
