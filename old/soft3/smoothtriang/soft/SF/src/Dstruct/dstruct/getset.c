...
static void BreakOffToken(char *From, char *Token, char *To);
static void EatLeadingBlanks(char **s);
static void StripBlanks(char* sFrom, char* sTo);
static void ProcessArray(char *AccessStr,Lnode **Adr, Lnode **NextAdr,
		  BOOLEAN *MakeNew, BOOLEAN *Error, BOOLEAN ForceMake);

static BOOLEAN	WasNV;	/* just says whether last guy was value from NV */
static Lnode	**FoundPtr;/* Global, storing the actual pointer to the
			      cell found in GetAdr() by a call that didn't
			   create a new cell. (Used in deletion) */

static Lnode	**FoundPtr2;/* yet another filthy hack: we need to go a further
...
*/
/* ForceMake says whether we make a new one when needed */
BOOLEAN	GetAdr(Lnode **Struct, char *String, Lnode **Adr, 
	       BOOLEAN ForceMake)
{
	...
	*Adr = FindNameInList(Token, Struct, &MakeNew, &Error, ForceMake);
	...
	while (*Work != '\0') {
		if (*Work == '[')	 {
			...
			/* The above should read through ], then index
			   into *Adr, and set NextAdr to be the new
			   value.  It should set MakeNew true iff it
			   had to make a new array element, and should
			   set Error to be true if there is a serious
			   problem.  At the return, Work must be
			   advanced to the new position to scan.  */
			...
			/* The above should ensure there's only 1 '.'
			   at the beginning, then it should break off
			   the token following it, and use it to
			   address into the structure pointed to by
			   *Adr.  It then must make NextAdr point to
			   the new place. Set MakeNew, Work, and Error
			   as above.  */
	...
	if ((*Struct == NULL) || ((*Struct)->position > i)) {
		if (!ForceMake)	 {
			*Error = TRUE;
			...
			/* maintains an ordered list coming from 
			   (*Struct)->cdr */
...
void ProcessValue(char *AccessStr,Lnode **Adr, Lnode **NextAdr,BOOLEAN *MakeNew, BOOLEAN *Error, BOOLEAN ForceMake)
{
	...
	if ((*Adr)->tag == VLIST) {
		FoundPtr2 = &((*Adr)->car);
		*NextAdr = FindNameInList(Token, &((*Adr)->car), 
					  MakeNew, Error, ForceMake);
	} else if ((*Adr)->tag == NV) {
		FoundPtr2 = &((*Adr)->cdr);
		*NextAdr = FindNameInList(Token, &((*Adr)->cdr), 
					  MakeNew, Error, ForceMake);
	} else {			/* so ((*Adr)->tag == NVLIST) */
		FoundPtr2 = Adr;
		*NextAdr = FindNameInList(Token, Adr, 
					  MakeNew, Error, ForceMake);
	}
}
...
Lnode *FindNameInList(char *GSName,Lnode **List,BOOLEAN *MakeNew, BOOLEAN* Error, BOOLEAN ForceMake)
{
...
BOOLEAN pGetScalar(Lnode **Struct,char *FieldSpec,Scalar *ScalarPtr)
{
	Lnode	*LookUp;
...
BOOLEAN pGetString(Lnode **Struct, char *FieldSpec,char **StringPtr)
{
	Lnode	*LookUp;
	BOOLEAN	FoundIt, Error = FALSE;
...
BOOLEAN pPutScalar(Lnode **Struct, char *FieldSpec,Scalar scalar)
{
	Lnode	*LookUp;
	BOOLEAN	FoundIt;
...
BOOLEAN pPutString(Lnode **Struct, char *FieldSpec,char *String)
{
	Lnode	*LookUp;
	BOOLEAN	FoundIt;
	char		*StringSpace;
...
BOOLEAN pCopyDstructFields(Lnode **d1, char *s1, Lnode **d2, char *s2)
   {
   BOOLEAN	Found;
...
void pDeleteLnode(Lnode **Struct)
{
	if (*Struct == NULL)
	  return;
...
BOOLEAN pDeleteField(Lnode **Struct, char *s1)
{
