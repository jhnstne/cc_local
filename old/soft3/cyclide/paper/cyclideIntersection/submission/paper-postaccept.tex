...
% They first note that the cyclide has a rational biquadratic parameterization
% \cite{MAR82}.
% If we substitute this biquadratic parameterization into the quartic
% equation of another cyclide, a degree 8 equation in $s$ and $t$ results.
% However, this equation is of degree 4 in $s$ and degree 4 in $t$.
% Thus, it can be solved symbolically for $t$, viewing $s$ as a constant.
...
% reason it is degree 8: two cyclides share double circle on the plane 
% 	at infinity, which removes a factor of 8
...
% It appears that circle/cyclide parameterization may be degree 4 as well,
% which would make the implicit/parametric method applicable in the
% context of ringed/cyclide as well.
% see circle-cyclide-may-be-possible in this directory.
...
Pratt \cite{P89,PRA89,Pratt90} 
	% P89 is the early version of Pratt90
	% PRA89 is a shorter version, mostly an overlap with Pratt90
...
\begin{figure}
\vspace{2in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/ring.ps
	hscale=75 vscale=75 voffset=-440}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/horn.ps
	hscale=75 vscale=75 hoffset=144 voffset=-455}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/horn2.ps
	hscale=75 vscale=75 hoffset=288 voffset=-448}
\caption{Cyclides: (a) Ring cyclide (b) Singly-horned cyclide 
		(c) Doubly-horned cyclide}
	% (b) Self-intersecting torus [13]
\label{cyc-pic}
\end{figure}
...
\begin{figure}
\vspace{5in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig2.ps
	hscale=75 vscale=75 voffset=170}
...
\begin{figure}
\vspace{2.5in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig3.ps
	 hoffset=-54}
...
This property will be examined in detail in Section~\ref{sec1}.
Pedoe \cite{Pedoe57} is an excellent source of proofs for Properties (1-5).
...
\begin{figure}
\vspace{2in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig4.ps
	 hoffset=-54}
...
% we only know how to invert a general cyclide with
...
The computation of the implicit equation of ${\rm cyclide}(t)$,
...
% \footnote{In particular, for each interval $(t_i,t_{i+1})$
%	delineated by these intersections, an arbitrary value of $t'$ can
%	be chosen and the center of inversion ${\rm circle}_2(t')(k)$
%	substituted into the implicit equation of the torus to determine
%	if it lies inside or outside the torus.
%	The entire interval $(t_i,t_{i+1})$ is outside the torus if and only if
%	the arbitrary $t' \in (t_i,t_{i+1})$ is.}
...
except at a finite number of points. % see *2 below
...
\Comment{
% 	It may be surprising that the line($t$)/cyclide($t$) intersections 
%	capture all of the roots of the circle($t$)/cyclide intersections.
	Since a circle can intersect a surface in more points 
	than a line, it behooves us to show that the intersection curve
	$\mbox{circle}(t) \cap \mbox{cyclide}$ is identical to the intersection
	curve $\mbox{inv}(\mbox{inv}(\mbox{line}(t) \cap \mbox{cyclide}(t)))$;
	that is, that the inversions do not lose some of the intersections
	(consider Figure~\ref{fig:box}).
% 	The inverse of a finite real point is a finite real point, unless
%	the point is the center of inversion.
	The only danger is that some of the points of intersection are mapped
	to infinity under the inversion, which can only happen if the 
	point of intersection is the center of inversion.  % *1 below
	This is clearly not a problem with the first inversion (Step 3):
	...
	Thus, at most a finite number of points
	of the intersection curve are lost,\footnote{As discussed in 
		Remark~\ref{rmk:noton}, 
		they are filled in as limits of their neighbours.}
	and the final line/cyclide intersection captures the same intersection
	curve as the circle/cyclide intersection.  
}
%	We conclude that some of the expected circle(t)/cyclide intersections 
%	are complex or at infinity. THEY COULD BE COUNTED TWICE.
% 	*1		
%	WILL THE PTS AT INFINITY MAP BACK TO FINITE PTS? Moot point, because 
% 	we won't compute any points at infinity when we are computing roots.
%
% 	*2
% 		A segment of the curve (circle(t))(k) might lie on the torus
% 		for some values of k, but not for all values of k.
% 		Thus, you need to check if torus(circle(t)(k)) has an 
% 		infinite number of solutions.
%		If so, choose a different value for k.
\end{rmk}
...
This is needed in step (5) of the intersection algorithm.
The inverse of a torus is an easy computation for the following reason.
...
\begin{figure}
\vspace{2.5in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig6.ps
	 hoffset=18 voffset=72 hscale=75 vscale=75}
...
\begin{figure}
\vspace{1.75in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig7.ps
 	voffset=45 hscale=75 vscale=75}
...
\begin{lemma}
\cite{J29} % p. 55
% also Pedoe57, p. 19 (defn of limiting point on p. 17)
\label{lem:cntr}
...
the diameter of $\mbox{inv}_S(D)$ is 
...
\begin{figure}
\vspace{2.5in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig8.ps
	 hoffset=18 voffset=72 hscale=75 vscale=75}
...
\begin{figure}
\vspace{2.25in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig9.ps
	 hoffset=54 voffset=60 hscale=75 vscale=75}
...
with centers $C_1$ and $C_2$.
	% another approach to finding the center of this circle is available 
	% in Miller and Goldman, who give a formula for the center of the 
	% intersection of two spheres (Fig 3 and p. 7 of this paper).
	% Of course, this doesn't use inversion, so it is not as appropriate.
Any plane containing the centers of the two spheres 
intersects the two spheres in two circles (Figure~\ref{fig:space-center}),
which intersect in $X$ and $Y$.
...
The only case that we have not dealt with is the inverse of a circle
...
found in Pedoe \cite{Pedoe57}. % p. 6
...
\Comment{
STEINER METHOD (NOT PRESENTED HERE, BUT A POTENTIAL METHOD)
...
% \figg{normal}{Surface normals and curve normals are not equivalent [29]}{2in}
% Fig 4.13, p. 102, Millman and Parker

\begin{figure}
\vspace{2in}
\special{psfile=/users/cogito/jj/Research/cyclide/figs/fig10.ps
	voffset=25 hscale=75 vscale=75}
...
} % end of commenting out of Section 12.1
