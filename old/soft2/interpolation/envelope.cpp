/*
  ...
  Purpose:       Build a set of samples from the envelope of an open input curve,
                 of radius R and sampling step size delta, using only the segment [t1,t2].
  Sequence:	 
  Input: 	 envelope's spine, as data points defining an open Bezier curve
  Status:        Used to build dynamic datasets for the explanation of local back umbral theory.
  Copyright 2004 by John K. Johnstone.
  History: 	 5/20/04: Built from interpolation.cpp template.
*/
...
static void usage()
 {
  ...
  cout << "\t <spineInput>.pts <envelopeOuput>.pts" << endl;
 }
...
static GLboolean DRAWCURVE=1;		// draw curved spines?
...
static GLboolean DRAWENVELOPEDATA=1;    // draw envelope probes?
static GLboolean DRAWENVELOPE=0;        // draw envelope curve?
...
V2fArr                  envelope;       // data points of envelope
  ...
  if (DRAWENVELOPEDATA)                           // envelope probes
    {
      if (PRINTOUT) glColor3fv (Black); else glColor3fv (Red);
      if (WINDOWS)
	for (j=0; j<envelope.getn(); j++)
	  drawPt (envelope[j][0], envelope[j][1]);
      else
	{
	  glBegin(GL_POINTS); 
	  for (j=0; j<envelope.getn(); j++) glVertex2f (envelope[j][0], envelope[j][1]); 
	  glEnd();
	}
    }
  if (DRAWENVELOPE)
...
int main (int argc, char **argv)
{
  ...
  ifstream infile;  infile.open(argv[argc-2]);  
  read (infile, Pt);
  spine.fit (Pt);
  ...
  int nSample = ceil((t2 - t1) / delta);
  envelope.allocate(2*nSample+2); // depends on sampling rate

  // first point is C(t1) - R C'(t1) / ||C'(t1)||
  spine.eval (t1, envelope[0]); 
  V2f tang; spine.evalTang (t1, tang);  tang.normalize();  tang *= R;
  envelope[0] -= tang;

  // halfway point is C(t2) + R C'(t2) / ||C'(t2)||
  spine.eval (t2, envelope[nSample+1]);
  spine.evalTang (t2, tang); tang.normalize(); tang *= R;
  envelope[nSample+1] += tang;

  // first set of points  is C(t) + R normal(t) / ||normal(t)||
  // second set of points is C(t) - R normal(t) / ||normal(t)||
  float t;
  for (i=0, t=t1; i<nSample; i++, t+=delta)  
    {
      spine.eval (t, envelope[i+1]);               // C(t)
      envelope[2*nSample+1 - i] = envelope[i+1]; 
      V2f N;  spine.evalNorm (t, N);  N.normalize(); N *= R;      // R normal(t) / ||normal(t)||
      envelope[i+1] += N;
      envelope[2*nSample+1 - i] -= N;
    }

  envCurve.fitClosed (envelope);
  envCurve.prepareDisplay (nPtsPerSegment, 0);

  ofstream outfile;  outfile.open(argv[argc-1]);  // output to a file
  outfile << "[ Envelope data generated by 'envelope.cpp' from the spine curve " 
	  << argv[argc-2] << "]" << endl;
  outfile << "{" << endl;
  for (i=0; i<envelope.getn(); i++)
    outfile << envelope[i][0] << " " << envelope[i][1] << endl;
  outfile << "}" << endl;
