/*
  ...
  Paper: 	 'Do you see it? Object visibility in a smooth flat scene'
                 J.K. Johnstone (2005)
  History: 	 
*/
...
int firstPtHit (int ob, V2f probeDirection); // done
...
int backtrack (BezierCurve2f &probe, BezierCurve2fArr &obstacle, int star, 
	       TangThruPtArrArr &tangThruView, V2iArr &extreme, 	       
...
int backtrack (BezierCurve2f &probe, BezierCurve2fArr &obstacle, int star, 
	       ...
	       V2f viewpt, float epsInt, float radiusRoom)
{
  V2f ptFirstHitA, ptFirstHitB; float tFirstHitA, tFirstHitB;
		   ...
		   V2f viewpt, float epsInt, float radiusRoom)
{
  ...
  while (!done)
	...
	{ witness[0] = viewpt; probeAlg[nProbe-1].getCtrlPt(1,witness[1]); return 1; } 
      /*
      if (backtrack (probeAlg[nProbe-1], obstacle, star, tangThruView, extreme,
		     B, oldB, 1, ptTangOnOldB, angExtreme,
		     probeAlg, probeOb, probePt,p robePtA, nProbe, witness, range, 
		     viewpt, epsIntersect, radiusRoom))
	return 1; 
      */
      ...
      else
	{
      ...
      if (!angRange[star].contains(angleProbe)) done=1;
    }
}
...
cout << "Computing all probes" << endl;
  ...
  // FIND ALL EXTREME AND PIERCING TANGENTS;
  // THEN FIGURE OUT HOW TO PROBE IN A DIRECTION FOR THE FIRST OBJECT;
  // THEN PROBE IN ALL THE EXTREME AND PIERCING DIRECTIONS.
...
if (level > 10)
{
  ...
  starIsVisible = isStarVisible(star, obstacle, tangThruView, extreme, piercing, 
				conInterval, angRange, angRangeCon, angExtreme, nProbe,
				probeAlg, probeOb, probePt, probePtA, witness, range,
  ...
  glutInitWindowPosition (1100,titleht);		// primal window
