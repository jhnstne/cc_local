...
\begin{lemma}[{\cite{abhy,abba3,walker}}]
...
\end{example}
%
\figg{4}{An illustration of Lemma 2}{2.25in}
...
Therefore, $s'$ must either lie outside of \wo's tangent or on \wwh\ (Figure~\ref{6}).
...
$\theta \in [\theta_{1,j},\theta_{2,j} + \pi]$ (?? could be $\ldots - \pi$ i think ??).
}
% *******************************************************************************
\end{theorem}
%
...
%
\Heading{Proof of Theorem~\ref{Tps}:}
...
\arc{x\infty} and \arc{a\infty}, where \arc{x\infty} (resp., \arc{a\infty}) is the part of
...
(A method for deciding whether \arc{AB} leaves A towards E or F is described in a footnote 
...
\end{lemma}
\Heading{Proof:}
Suppose that $s$'s convex segment crosses \seg{sx} at $y$ (Figure~\ref{fig-27a}).
...
and \arc{xz} does not contain any endpoints in its interior (Figure~\ref{fig645}).
...
\begin{lemma}[{\cite[p. 119]{jj}}]   % Lemma B.8, p. 119
\label{Ll}
