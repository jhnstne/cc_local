% SUBMIT TO BIT (OR JOURNAL THAT CRAWLING PAPERS WERE PUBLISHED IN);  ALGORITHMICA?
% THE JOURNAL VERSION MAY DIFFER BY INCLUDING DISCUSSION OF CONVEX XY-MONOTONE SEGMENTS
% SEE \Comment{FOR JOURNAL VERSION:  ...}'S
\newif\ifFull
% \Fullfalse		
\Fulltrue		
...
Dr. Michael T. Goodrich ((301) 338-7708) is an assistant professor of computer science at 
...
One would hope, then, for a method that made segment intersection simpler than curve 
...
% AREN'T THERE OTHER SITUATIONS THAT WE USE THE EPSILON ASSUMPTION THAT CANNOT BE SO
% EASILY REMOVED?
...
\begin{lemma}[Canny's gap theorem{ \cite{canny} }]
% p.54
\label{thm-gap}
...
\Comment{
It should be noted that the point $P_{\mbox{\footnotesize{active}}}$ when the relative 
order switches is the best 
approximation to the intersection under the granularity of the crawl.
(This becomes clear when one notices that each crawl is restricted by both the
x and y value of the other segment, and the relative order of either x or y must switch
as an intersection is crossed.)

Another way of seeing this.
	Consider two vertical lines in figure~\ref{} and the shallower segment between them.
	the distance between the lines is at most the horizontal length of a crawl step 
 	along the shallower segment.  it is clear that the intersection lies in between 
	two vertical lines.  thus, it lies on the last crawl step.
}
...
\Comment{
SUGGEST STAIRCASE CRAWL AS AN ALTERNATIVE BECAUSE IT OFFERS LONGER CRAWLS 
	(WHICH MAKES IT MORE LIKELY THAT LINE-CURVE INTERSECTIONS WILL PAY OFF AS 
	 REPLACEMENTS FOR SOME CRAWLS)
}
...
will result (Figure~\ref{fig.stair} and Lemma~\ref{lem-converge}).
The associated switching condition is
\begin{eqnarray*}
...
the segments becomes less than $\epsilon$ (Figure~\ref{fig.epsilon.stair}).
...
% because a switch is made *as soon as*
% $x(P_{\mbox{\footnotesize{active}}})\ >\ x(P_{\mbox{\footnotesize{dormant}}})$.
...
% An xy-monotone decomposition will involve finding AND SORTING THE CURVE'S LOCAL EXTREMA.
% % in order to pair them into segment endpoints
% A curve of degree $n$ will have at most $O(n^{2})$ xy-monotone segments, since there are
% $O(n^{2})$ local extrema.

\Comment{
First of all, the singularities of an algebraic curve are fundamental to
many geometric modeling algorithms (e.g., \cite{abba,johnstone87}),
and it is trivial to determine the extrema of a curve as part of the singularity 
...
\Comment{
In other words, we are willing to incur a preprocessing expense if this
will allow us to speed up all future intersections with this curve.
The expense of preprocessing is well warranted for curves in a geometric model,
...
xy-monotone decomposition by crawling is probably the best choice.

\Comment{
MAYBE INCLUDE IN JOURNAL VERSION
...
to the segment at P (Figure~\ref{fig.decompose}).
...
(Figure~\ref{fig.exceptions}); but it works fine when combined with the marking of points
...
\Comment{
POSSIBLY FOR JOURNAL VERSION: HOW TO ENTIRELY AVOID A SEGMENT-SEGMENT INTERSECTION
...
\Comment{
THE ONLY WAY THAT COORDINATED CRAWLING COULD SHED SOME REALLY UNEXPECTED LIGHT WOULD BE 
IF ONE COULD AVOID CRAWLING ALONG THE ENTIRE SEGMENTS, OTHERWISE THIS IS MORE OF A 
`REMEMBER THAT THIS IS AN ALTERNATIVE' PAPER RATHER THAN A PAPER WITH REVEALING INSIGHTS.
	LINE-CURVE INTERSECTIONS ARE ONE WAY
	THE FACT THAT TWO CONVEX SEGMENTS HAVE AT MOST TWO INTERSECTIONS IS ANOTHER
		WAY OF SKIPPING PAST SOME OF THE CURVE AUTOMATICALLY
	THE FACT THAT A RISING AND FALLING SEGMENT HAVE AT MOST ONE INTERSECTION IS ANOTHER
		WAY OF SKIPPING PAST SOME OF THE CURVE AUTOMATICALLY
	THE CONDITIONS THAT PROHIBIT INTERSECTION ALLOW TWO SEGMENTS TO BE THROWN OUT 
		RIGHT AWAY AS WELL
	N.B. DON'T HAVE TO INTERSECT EVERY PAIR OF XY-MONOTONE SEGMENTS, ONLY THOSE
		THAT OVERLAP IN X; MOST OF THESE WON'T OVERLAP IN Y
	MORAL: AFTER XY-MONOTONE (CONVEX?) DECOMPOSITION (WHICH IS STRAIGHTFORWARD
		PROCESSING OF EACH SEGMENT INDEPENDENTLY)
		COMPLEXITY OF FINDING INTERSECTIONS WITH COORDINATED CRAWLING IS
		PROBABLY EVEN LESS THAN THE COMPLEXITY OF CRAWLING ALONG BOTH SEGMENTS
}
...
% Viewed in this way, each crawl of the staircase crawl is a recursive call to the 
% crawl method of intersection.
...
\Comment{
PERHAPS FOR JOURNAL VERSION

CLAIM:  The crawls of a staircase crawl tend to be longest on convex segments.
	Thus, it is more likely that line-curve intersections will be useful replacements
	on convex segments.
}
...
Suppose that we are intersecting the xy-monotone segment \arc{AB} and \arc{CD} on curves
...
The method may be impractical for entire algebraic curves, since there are O($n^3$) 
...
\Comment{
BUT THEN XY-MONOTONE DECOMPOSITION OF THE CURVES WOULD BE PROHIBITIVE AS WELL
}
...
%\bibitem{Good87}
%`Optimal hidden-line elimination', Tech report
...
\bibitem{Mck86} M.~McKenna, ``Worst-case Optimal Hidden-Surface Removal,''
...
\bibitem{SeG82} S.~Sechrest and D.P.~Greenberg, ``A Visibility
Polygon
...
\figg{pic.crawl}{Crawling along a curve}{1.5in}

% SIMULTANEOUS CRAWL SECTION

\figg{fig.stair}{(a-b) Two staircase crawls}{4.5in}

\figg{fig.simul}{(a) a simultaneous crawl (b) at the end, it must back up finely}{3.5in}

\figg{fig.counter2}{The danger of a simultaneous crawl along two rising segments}{2in}

\figg{fig.skip}{The danger of a staircase crawl that does not back up one step
		after each crawl}{2.3in}

			        % picture is in skipover.pic

% \figg{fig.epsilon.stair}{An intersection is not signalled here}{.5in}

\figg{eps.zone}{$\epsilon$-zones}{2in}

\figg{countereg}{A staircase crawl cannot be used for a rising and a falling segment}{1.5in}

				% picture is in countereg.pic

\figg{fig.delta}{\arc{AB} and \arc{CD} cannot intersect}{2.25in}

\figg{fig.triangle}{The length of \arc{AB} is bounded by $a+b$}{2in}

\figg{fig.binary}{A binary search for the intersection}{2.6in}

\figg{fig.pleft}{A curve comparison between $C'$ and p: crawl from q to r}{2.5in}
