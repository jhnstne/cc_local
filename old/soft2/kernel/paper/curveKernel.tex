...
% Computing the kernel of a curve from the convex hull}
% A dual computation of the kernel of a plane curve}
...
\begin{figure}[h]
\begin{center}
% \includegraphics*[scale=.25]{img/jjhu1.jpg}
...
% a point inside the kernel is not hit by any tangent.\footnote{A boundary point
% of the kernel is hit either by its own tangent or by the tangent of an inflection point.}
...
% We call $C^*(t)$ a tangential curve, as it represents a tangent space.
...
% In the special case when the origin lies inside the kernel of $C$,
% the tangential c-curve is a robust representation of the tangent space of $C$.

% When a single point of $C$'s kernel is known,
% another dual representation of $C$'s tangent space is possible.\footnote{
% After translating the kernel point to the origin,
% the tangential c-curve (alone) is a robust representation of the tangent space of $C$.
...
% lies inside \nolinebreak $C$'s \nolinebreak \mbox{kernel}.\footnote{This 
...
and convex hull.
% \footnote{The convex hull of $C$ is the smallest convex set that
%	contains $C$.  See \cite{preparata85}.}
...
free tangents of $C^*$ (extremal free lines) % that are just about to intersect $C^*$)
...
% (The convex hull of an infinite tangential curve bears no relationship to the kernel.)
...
% Be careful about convex curves C: kernel but no inflection points,
% so $C^*$ has no cusps so no cusp bitangents.

\begin{lemma}
\label{lem:allcusp}
Let $C$ be a curve with nonempty kernel.
All of the bitangents of its tangential curve $C^*$ are cusp bitangents.
% (Figs.~\ref{fig:kernelob1a},\ref{fig:banana}).
\end{lemma}
\ifACM
\begin{proof}
\else
\prf
\fi
% We begin by examining the structure of the bitangents of a tangential curve.
% If the kernel of $C$ is empty, no lines in dual space are free 
% (Lemma~\ref{lem:dualkernelchar}) and the result is vacuously true.
% Suppose that the kernel is not empty.
...
% Lemma~\ref{lem:allcusp} also allows the search for a free line to be restricted.
...
% This is always possible because it involves cusps.
% Is this important(?):
...
% Reference: Sabin, Mathematics of Surfaces, duality paper.
...
% \begin{corollary}
% The search for a kernel point of $C$ in primal space
% can be reduced to a search for cusp bitangents of $C^*$ in dual space,
% followed by a test for freedom.
% \end{corollary}
...
% All of this computation is performed in both dual spaces,
% constrained to $y \in [-1,1]$ in a-dual space and $x \in (-1,1)$ in b-dual space.
% A cusp bitangent must be free in both dual spaces to be considered free
% (Definition~\ref{defn:linesystem}).
...
\Comment{
% If you do this, you may as well compute the intersection of inflection halfspaces,
% since you are computing inflections and their intersections already.

\noindent This algorithm can also be moved to primal space,
using the fact that the dual of a cusp is the tangent of an inflection point
and the following lemma.

\begin{lemma}
The line between the points $a$ and $b$ 
dualizes to the intersection of the lines $a^*$ and $b^*$.
\end{lemma}
% \footnote{An inflection
%	point involves a reversal of the tangent's direction, 
%	dual to the cusp's reversal of direction.}

\centerline{{\bf Algorithm 3: Computing a seed point of the kernel another way}}

\begin{enumerate}
\item	Compute the inflection points of $C$ and their tangents.
	If there are none, $C$ must be convex and its kernel is the interior of $C$.
\item	Compute the intersections of every pair of inflection tangents.
	These are the duals of the lines through two cusps
	(one of the types of cusp bitangent).
\item 	Compute the intersections of the inflection tangents with $C$.
	These are the duals of the tangents through a cusp
	(the other type of cusp bitangent).
\item	Dualize these intersections and test for freedom.
\item	The intersections corresponding to free lines are kernel points of $C$.
\end{enumerate}
}
...
% We now observe that we can do a bit better.
% However, we can do better.
% Algorithm 1 can be optimized by computing the kernel
% directly from the computations of Algorithm 2.
% Since the free bitangents of a curve define its convex hull,
...
% Since the kernel is convex, its corners are sufficient for its construction.

% The theoretical development of Section~\ref{sec:first} reveals a stronger result:
% the set of kernel points associated with the free cusp bitangents
% actually define the entire kernel.
% Recall that the free tangents of $C^*$ correspond to the boundary of the kernel 
% (proof of Theorem~\ref{thm:kerneldual}).
% As extremal free tangents, free bitangents define the corners of this boundary,
% which define the convex kernel (Figures~\ref{fig:kernelob1a}-\ref{fig:banana}).
% This leads to the following alternative to Algorithm 1 for computing the kernel.
...
% This can also be understood by observing that the free cusp bitangents
% define the convex hull of $C^*$, when it is finite.\footnote{The convex 
%	hull is defined by bitangents and curve segments between these bitangents.}
% Naturally, their duals define the kernel as well.
