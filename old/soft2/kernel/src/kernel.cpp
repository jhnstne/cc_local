/*
  ...
  Created:	 23 August 2002 (from tangentialCurve.cpp)
  Last Modified: 28 February 2006
  Purpose:       Compute the kernel of the given curve.
		 Builds on tangentialCurve.cpp.
  Sequence: 	 3rd in a sequence (interpolate, tangentialCurve, kernel)
  ...
  History: 	 8/23/02: Added mouse-tracking capability for analysis
			  ...
			  Added tangential c-curve.
		 8/27/02: Computed inflection points.
		 8/28/02: Computed inflection tangent intersections.
		 9/1/02:  Transition to clipping around pts of tangency
		 	  through origin in primal space.
		 9/4/02:  Computing full kernel from finite kernel points.			  
		 9/13/02: Drawing convex hull in dual space (cheating: using kernel).
		 9/25/02: Drawing trim region.
...
#include "curve/BezierCurve.h"	// drawTangent, drawPt
#include "tangcurve/TangCurve.h" // create; evalProj, drawCtrlPoly, drawPt (from RatBezierCurve inheritance)
...
static GLboolean DRAWCLIP=0;		// emphasize clipped out segments of primal curve?
static GLboolean DRAWCENTROID=0;	// draw centroid of found kernel pts?
static GLboolean DRAWMARKKER=0;		// draw kernel points, marked as hitInfl or hitCurve?
static GLboolean DRAWKERNEL=1;		// draw kernel?
static GLboolean DRAWHULL=0;		// draw convex hull in dual space?
static GLboolean DRAWTRIMREGION=0;	// draw one trimmed region?
...
TangentialCurve		obdualc;	// associated tangential c-curve
...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case 'c':	DRAWCLIP = !DRAWCLIP;		break;
  case 'o':	DRAWORIGIN = !DRAWORIGIN;	break;
  case 'C':	DRAWCENTROID = !DRAWCENTROID;	break;
  case 'm':	DRAWMARKKER = !DRAWMARKKER;	break;
  case 'K':	DRAWKERNEL = !DRAWKERNEL;	break;
  case 'i':	DRAWINFL = !DRAWINFL;		break;
  case 'R':	DRAWTRIMREGION = !DRAWTRIMREGION; break;
  case 'h':	DRAWHULL = !DRAWHULL;		break;
  default:      break;
  }
  glutPostRedisplay();
}
...
void menuOb (int value)
{
  ...
  case 13:	DRAWCLIP = !DRAWCLIP;		break;
  case 14:	DRAWCENTROID = !DRAWCENTROID;		break;
  case 15:	DRAWMARKKER = !DRAWMARKKER;	break;
  case 16:	DRAWKERNEL = !DRAWKERNEL;	break;
  case 17:	DRAWINFL = !DRAWINFL;		break;
  case 18:	DRAWTRIMREGION = !DRAWTRIMREGION; break;
  default:   					break;
  }
  glutPostRedisplay();
}
...
void menuDual (int value)
{
  ...
  case 5:     DRAWALLDUAL = !DRAWALLDUAL;			break;
  case 6:     DRAWHULL = !DRAWHULL;				break;
  default:   							break;
  }
  glutPostRedisplay();
}
...
//  if (PRINTOUT) glColor3fv (greyscale[0]); else 
  ...
  if (DRAWCLIP)		// highlight the clipped out segments
   {
    glLineWidth (3.0);
    glColor3fv (Black);
    obstacle.drawInactive (obdualc.active, nPtsPerSegment);
    glLineWidth (1.0);
   }
...
//    if (PRINTOUT) glColor3fv (Black); else 
  ...
  if (DRAWDUAL)
   {
    glColor3fv (Blue);
    V3f ptDual;  obdualc.evalProj (tActive, ptDual);
...
//    if (PRINTOUT) glColor3fv (Black); else 
    glColor3fv (Blue); 
    V2f pt;   obdualc.eval     (tActive, pt);
    V2f tang; obdualc.evalTang (tActive, tang);
    ...
    glBegin(GL_POINTS);			
    glVertex2f (dualTang[0], dualTang[1]);
    glEnd();
    ...
    glutBitmapCharacter(GLUT_BITMAP_9_BY_15, ')');
// cout << "Tang in dual space  : (1, "
//      << -tang[0]/tang[1] << ", " << -(pt[0]*tang[1] - pt[1]*tang[0])/tang[1] << ")" << endl;
   }
  if (DRAWDUALOFMOUSE)
   {
    glColor3fv(Black);
...
//      if (PRINTOUT) glColor3fv (Black); else
//       {
        if (kernelSortIndex[i] < nKernelFromInfl) glColor3fv (Magenta); 
	else					  glColor3fv (Chocolate);
//       }
      if (WINDOWS)  drawPt (hitKernel[i][0], hitKernel[i][1]);
      else { glBegin (GL_POINTS); glVertex2f (hitKernel[i][0], hitKernel[i][1]); glEnd(); }      
     }
...
//    if (PRINTOUT) glColor3fv (Black); else
    glColor3fv (Magenta);
    if (WINDOWS)  drawPt (kernelCentroid[0], kernelCentroid[1]);
    else { glBegin (GL_POINTS); glVertex2f (kernelCentroid[0], kernelCentroid[1]); glEnd(); }    
   }
  if (DRAWKERNEL)
   {
    glColor3fv (Black);
    if (hitKernel.getn() > nKernelFromInfl)	// there are some active segments
      kernelCurve.drawActive (activeKernel, nPtsPerSegment);
    for (i=0; i<hitKernel.getn(); i++)
      if (kernelSortIndex[i] < nKernelFromInfl ||
      	  kernelSortIndex[(i+1)%hitKernel.getn()] < nKernelFromInfl)
       {
        glBegin(GL_LINES);
	glVertex2f (hitKernel[i][0], hitKernel[i][1]);
	glVertex2f (hitKernel[(i+1)%hitKernel.getn()][0], hitKernel[(i+1)%hitKernel.getn()][1]);
	glEnd();
       }
   }
  if (DRAWTRIMREGION)		// draw one trim region (about tTangThruOrigin[0])
   {
    glColor3fv (Black);
    obstacle.drawTangent (tTangThruOrigin[0], 	    hodo0, 1);

    glColor3fv (Grey);
    Line2f L1,L2;
    obstacle.drawTangent (tTangThruOrigin[0] - .05, hodo0, 1);	// .05 is passed to createC for trimming
    obstacle.drawTangent (tTangThruOrigin[0] + .05, hodo0, 1);

    obstacle.buildTangent(tTangThruOrigin[0] - .05, hodo0, 1, L1);
    obstacle.buildTangent(tTangThruOrigin[0] + .05, hodo0, 1, L2);
    V2f hit; float foo;    L1.intersect (L2, hit, foo, 1);
//    if (WINDOWS)  drawPt (hit[0], hit[1]);
//    else { glBegin (GL_POINTS); glVertex2f (hit[0], hit[1]); glEnd(); }    
    
    V2f P1,P2;				// points of tangency
    obstacle.eval (tTangThruOrigin[0] - .05, P1);
    obstacle.eval (tTangThruOrigin[0] + .05, P2);
    int insideL2 = L2.inside (P1);	// 'inside' value for L2
    int insideL1 = L1.inside (P2);	// 'inside' value for L1
    V2f endL1InsideL2, endL1OutsideL2;	// classify the endpoints of L1
    if (L2.inside(L1[0]) == insideL2)
         { endL1InsideL2 = L1[0]; endL1OutsideL2 = L1[1]; }
    else { endL1InsideL2 = L1[1]; endL1OutsideL2 = L1[0]; }
    V2f endL2InsideL1, endL2OutsideL1;	// classify the endpoints of L2
    if (L1.inside(L2[0]) == insideL1)
         { endL2InsideL1 = L2[0]; endL2OutsideL1 = L2[1]; }
    else { endL2InsideL1 = L2[1]; endL2OutsideL1 = L2[0]; }
    
    glPolygonMode (GL_FRONT_AND_BACK, GL_FILL);
    glBegin (GL_POLYGON);
    glVertex2f (endL1InsideL2[0], endL1InsideL2[1]);
    glVertex2f (hit[0], hit[1]);
    glVertex2f (endL2OutsideL1[0], endL2OutsideL1[1]);
    glEnd();
    glBegin (GL_POLYGON);
    glVertex2f (endL1OutsideL2[0], endL1OutsideL2[1]);
    glVertex2f (hit[0], hit[1]);
    glVertex2f (endL2InsideL1[0], endL2InsideL1[1]);
    glEnd();
   }
...
//  if (PRINTOUT) glColor3fv (Black); else
  glColor3fv (colour[0]);
  obdualc.drawT (nPtsPerSegment*5);
//  glColor3fv (Black);
//  obdualc.drawCtrlPoly();
  if (DRAWALLDUAL)
   {
//    if (PRINTOUT) glColor3fv (greyscale[0]); else 
    glColor3fv (colour[0]);
...
//    if (PRINTOUT) glColor3fv (Black); else 
    glColor3fv (Red); 
    if (WINDOWS) { V2f p; obdualc.eval(tActive,p); drawPt(p[0],p[1],.05); }
    else 		  obdualc.drawPt(tActive); 
   }
  if (DRAWDUALTANG)
   {
//    if (PRINTOUT) glColor3fv (Black); else 
    glColor3fv (Red); 
    obdualc.drawTangent (tActive);
    V2f p; obdualc.eval(tActive,p);		// label point Q
...
// cout << "Dual of active point: (1, " << p[1] << ", " << p[0] << ")" << endl;    
   }
  if (DRAWDUALOFMOUSE)
   {
    glColor3fv (Black);
    drawImplicitLine (xMouse,yMouse,1);
   }
  if (DRAWINFLTANG)		// draw duals of inflection tangents
   {
    glColor3fv (Black);
    for (i=0; i<tInfl.getn(); i++) obdualc.drawPt (tInfl[i]);
   }
  if (DRAWINFLHIT)
   {
//    if (PRINTOUT) glColor3fv (Black); else
    glColor3fv (Green);
    for (i=0; i<hitInfl.getn(); i++)
      drawImplicitLine (hitInfl[i][0], hitInfl[i][1], 1);
   }
  if (DRAWCURVEHIT)
   {
//    if (PRINTOUT) glColor3fv (Black); else
    glColor3fv (Blue);
    for (i=0; i<hitCurve.getn(); i++)
      drawImplicitLine (hitCurve[i][0], hitCurve[i][1], 1);
   }
  if (DRAWKERHIT)
   {
//    if (PRINTOUT) glColor3fv (Black); else
    glColor3fv (Magenta);
    for (i=0; i<hitKernel.getn(); i++)
      drawImplicitLine (hitKernel[i][0], hitKernel[i][1], 1);
   }
  if (DRAWHULL)
   {
    glColor3fv (Black);
    glBegin(GL_LINES);
    for (i=0; i<hitCurveParameterPair.getn(); i++)
     {
      V2f p, q;
      obdualc.eval (hitCurveParameterPair[i][0], p);
      obdualc.eval (hitCurveParameterPair[i][1], q);
      glVertex2f (p[0], p[1]);
      glVertex2f (q[0], q[1]);
     }
    glEnd();
...
int main (int argc, char **argv)
{
  ...
  obdualc.createC (obstacle, 0, .05);	// compute tangential c-curve
// obdualc.print();
// cout << obdualc.active << endl;
  obstacle.prepareDisplay (nPtsPerSegment);	// postpone, since obstacle changes in creating obdualc
  obdualc.prepareDisplay (nPtsPerSegment*5);
...
// cout << hit.getn() << " intersections of inflection tangent with curve" << endl;    
  ...
  /******* compute intersections associated with free lines in dual space ****/

  IntArr hitKerFlag (hitInfl.getn() + hitCurve.getn());
  int nHitKer=0;	
  TangentialCurve hitDual;	// dual of intersection point
  V2fArr fooHit; FloatArr tFoo, btFoo;
  for (i=0; i<hitInfl.getn(); i++)
   {
cout << "Considering hitInfl[" << i << "]" << endl;   
    hitDual.createImplicitLine (hitInfl[i][0], hitInfl[i][1], 1);
    obdualc.intersect (hitDual, nHit, fooHit, tFoo, btFoo, eps);

    	// ignore intersections near 'cusps' defining the line
    V2f dualPt1; obdualc.eval (linePair[i][0], dualPt1);	// 'cusps'
    V2f dualPt2; obdualc.eval (linePair[i][1], dualPt2);
    int nIgnore=0;		// how many intersections should be ignored?
    for (j=0; j<nHit; j++)
     {
// cout << fooHit[j].dist (dualPt1) << " " << fooHit[j].dist (dualPt2) << endl;
      if (fooHit[j].dist (dualPt1) < 10*eps || fooHit[j].dist (dualPt2) < 10*eps) 
        nIgnore++;
     }

// cout << nHit - nIgnore << endl;
    if (nHit - nIgnore == 0)		// dual of hitInfl[i] is free
     {   hitKerFlag[i] = 1; nHitKer++; }
    else hitKerFlag[i] = 0;
   }
  ...
  for (i=0; i<hitCurve.getn(); i++)
   {
cout << "Considering hitCurve[" << i << "]" << endl;   
    hitDual.createImplicitLine (hitCurve[i][0], hitCurve[i][1], 1);
    obdualc.intersect (hitDual, nHit, fooHit, tFoo, btFoo, eps);  

    	// ignore intersections near cusp and curve point defining the line
    V2f dualPt1; obdualc.eval (parameterPair[i][0], dualPt1);
    V2f dualPt2; obdualc.eval (parameterPair[i][1], dualPt2);
    int nIgnore=0;		// how many intersections should be ignored?
    for (j=0; j<nHit; j++)
     {
// cout << fooHit[j].dist (dualPt1) << " " << fooHit[j].dist (dualPt2) << endl;
...
// cout << nHit - nIgnore << endl;
    if (nHit - nIgnore == 0)		// dual of hitCurve[i] is free
     {   hitKerFlag[hitInfl.getn() + i] = 1; nHitKer++; }
    else hitKerFlag[hitInfl.getn() + i] = 0;
   }

cout << "Collecting" << endl;   
  hitKernel.allocate (nHitKer);		// copy over
  IntArr kernelIndex(nHitKer);		// remember where each kernel pt came from
  nHitKer = 0;
  for (i=0; i<hitInfl.getn(); i++)
    if (hitKerFlag[i]) 	
     {
      kernelIndex[nHitKer] = i;
      hitKernel[nHitKer++] = hitInfl[i];
     }
  nKernelFromInfl = nHitKer;	// record # of kernel pts from hitInfl
  hitCurveParameterPair.allocate (hitKernel.getn() - nKernelFromInfl);
  for (i=0; i<hitCurve.getn(); i++)
    if (hitKerFlag[hitInfl.getn() + i]) 
     {
      kernelIndex[nHitKer] = i;
      hitCurveParameterPair[nHitKer - nKernelFromInfl] = parameterPair[i]; 
      hitKernel[nHitKer++] = hitCurve[i];
     }
     
if (hitKernel.getn() > 0)	// nonempty kernel
 {
cout << "Building full kernel from finite kernel points" << endl;
  // build full kernel:
  // connect two consecutive points of sorted hitKernel
  // by a straight line if one of these points is an intersection of inflection tangents, and
  // by a curve segment otherwise (both points are intersections of curve with inflection tangent)

  for (i=0; i<hitKernel.getn(); i++) 	// compute sample mean
    kernelMean += hitKernel[i];
  kernelMean /= hitKernel.getn();
  
  int nKernelFromCurve = hitKernel.getn() - nKernelFromInfl;
  if (nKernelFromCurve > 0)	// some kernel pts from curve
   {
    kernelCurve = obstacle;	// subdivide kernel curve at hitCurve points of hitKernel
    FloatArr tCurve(hitKernel.getn() - nKernelFromInfl);
    for (i=0; i<tCurve.getn(); i++) 	// first few in kernel are from tInfl
      tCurve[i] = parameterPair[kernelIndex[nKernelFromInfl+i]][1];
    IntArr foo;  tCurve.bubbleSort (foo);
    kernelCurve.subdivideSpline (tCurve);
  
//  kernelCurve.print();
  
    // sort by angle about mean (first determine sort direction)
    assert (tCurve[0] + .1 < kernelCurve.getLastKnot());
    V2f pt0; kernelCurve.eval (tCurve[0],       pt0);
    V2f pt1; kernelCurve.eval (tCurve[0] + .01, pt1);
    V2f vec0(pt0);  vec0 -= kernelMean;
    V2f vec1(pt1);  vec1 -= kernelMean;
    if (vec0.angleFromX() < vec1.angleFromX())
         sortByAngleCCW (hitKernel, kernelMean, kernelSortIndex);  
    else sortByAngleCW  (hitKernel, kernelMean, kernelSortIndex);

    // prepare display of kernel segments
    activeKernel.allocate(kernelCurve.getL());
    activeKernel.clear();			// initially turn off all segments
    for (i=0; i<hitKernel.getn(); i++)		// traverse kernel boundary
      if (kernelSortIndex[i]   >= nKernelFromInfl &&	// consecutive hitCurve points
          kernelSortIndex[(i+1)%hitKernel.getn()] >= nKernelFromInfl)
       {
          // turn on segments between hitKernel[i] and hitKernel[i+1]
          // 	since this represents a segment of the kernel
          // find parameter values of two endpoints
        float tStart = parameterPair [kernelIndex[kernelSortIndex[i]]  ][1];
        float tEnd   = parameterPair [kernelIndex[kernelSortIndex[(i+1)%hitKernel.getn()]]][1];
// cout << "Turning on from " << tStart << " to " << tEnd << endl;     
        j=0;  while (kernelCurve.getKnot(j) != tStart) j++;
        while (kernelCurve.getKnot(j) != tEnd)
         {
// cout << "Turning on segment (" << kernelCurve.getKnot(j) << "," 
//			       << kernelCurve.getKnot(j+1) << ")" << endl;
	  if (j != kernelCurve.getL())			       
            activeKernel[j] = 1;
	  if (j == kernelCurve.getL()-1) 	
	    if (tEnd == kernelCurve.getKnot(j+1))	// stop at end of curve
	       j=j+1;
	    else					// wraparound
	       j=0;
	  else j++;
    ...
    kernelCurve.prepareActiveDisplay (activeKernel, nPtsPerSegment);
  	// can't inherit obstacle's display, since we have subdivided at kernel points
   }
  else 	// sort direction doesn't matter
    	 sortByAngleCCW (hitKernel, kernelMean, kernelSortIndex); 
  
  //  centroid requires sorted polygon
  Polygon2f kernelBdry (hitKernel);
  kernelBdry.centroid (kernelCentroid);
}  // if (hitKernel.getn() > 0)
  ...
  glutInitWindowPosition (dualxleft,cdualy);		// c-dual window
  glutInitWindowSize (xsize,ysize);
  strcpy (titlebar, "Tangential c-curves");
  ...
  glutAddMenuEntry ("Unclipped tangential c-curve", 	5);
  glutAddMenuEntry ("Convex hull [h]",			6);
  glutAttachMenu (GLUT_RIGHT_BUTTON);
