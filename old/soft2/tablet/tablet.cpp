...
#define WINDOWS 1		 // running under Windows?
...
int xleft=0, xright=1650,               // window corners (in window coordinates)
    ybot=1100, ytop=0;
float xcenter=825,ycenter=550;          // center of window (in window coordinates)
  ...
  case GLUT_LEFT_BUTTON:
	...
	case GLUT_DOWN: 
	  ...
	  if (firstClick)
	    ...
	    {
	      Pt[nCurve][nPt[nCurve]][0] = (x-xcenter) / (xsize/6.); // (x-xcenter)/250.;
	      Pt[nCurve][nPt[nCurve]][1] = (ycenter-y) / (ysize/4.); // (ycenter-y)/250.;
	      if (firstPtOfCurve)
		{
		  outfile << "{" << endl;
		  firstPtOfCurve = 0;
