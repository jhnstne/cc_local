...
Array<RatBezierCurve2f> obdual2;	// type-2 dual curves of obstacles
int			nProbe;		// debugging of dual intersection
Probe 		       *probe;  
int			activeProbe=0;	// display which intersection probe?
int			nHit;		
...
/*case ' ': 	activeProbe = (activeProbe+1)%nProbe;	
  		cout << probe[activeProbe].a[0][0] << " " 
		     << probe[activeProbe].a[0][1] << endl;
  		break;
  case 8: 	activeProbe--; if (activeProbe < 0) activeProbe = nProbe-1; break; */
  default:      break;
  }
  glutPostRedisplay();
}
...
/*    V3f pt;  obdual[0].evalProj (tActive, pt);	// (b,c,a)
    drawImplicitLine (pt[2],pt[0],pt[1]);	// ax + by + c = 0
    glColor3fv (Green);
    	     obdual2[0].evalProj (tActive, pt); // (a,c,b)
    drawImplicitLine (pt[0],pt[2],pt[1]);  	// ax + by + c = 0 */
   }
...
void displayDual2 ()
{
  ...
  glPolygonMode (GL_FRONT_AND_BACK, GL_LINE);
  if (DRAWDUALCURVE) { glColor3fv(Red);	obdual2[0].draw(); glColor3fv(Blue); obdual2[1].draw();}
  if (DRAWDUALCTRLPOLY) { glColor3fv (Blue); for (i=0;i<n;i++) obdual2[i].drawCtrlPoly(); }
  ...
  if (!JUSTVIEWING)			
   {
    ...
    for (i=0; i<n; i++)  
     {
      obdual[i].oldDualOf (obstacle[i], tHitZeroWt[i]);
      obdual[i].prepareDisplay (nPtsPerSegment*30);
obdual[i].print();
     }
    // no doubt, no longer necessary (error was in tangent computation, not point at infinity problem)
    obdual2.allocate(n);
    Array<FloatArr> foo(n);
    for (i=0; i<n; i++)
     {
      obdual2[i].oldDualOf (obstacle[i], foo[i]);
      obdual2[i].prepareDisplay (nPtsPerSegment*30);
     }

    /***************compute intersections of duals************************/
    int nHitOrig;	V2fArr hitOrig;
    FloatArr tHitOrig0, tHitOrig1;
    probe = new Probe[10000]; 		// debug intersection
    obdual[0].intersectWithIgnoredSegments (obdual[1], nHitOrig, hitOrig, tHitOrig0, tHitOrig1,
    			      tHitZeroWt[0], tHitZeroWt[1], .0001); // .0001
//    			      probe,nProbe);
		cout << nProbe << " intersection probes" << endl;
...
*/    
    ...
    cullMistakes (nHit,     hit,     tHit0,     tHit1, 
    		  nHitTrue, hitTrue, tHit0True, tHit1True, 0);	// also depends on hodo0,hodo1,obstacle
...
//    tActive = obstacle[0].getKnot(0);		// start at beginning
  ...
  glutInitWindowPosition (724,302);		// type-2 dual window
  glutInitWindowSize (545,272);
  strcpy (titlebar, "Type 2 dual curves");
