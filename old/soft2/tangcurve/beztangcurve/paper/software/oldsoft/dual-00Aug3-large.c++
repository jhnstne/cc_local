  ...
  if (DRAWDUALCURVE)
   {
    ...
    if (!ONEOBSTACLE)
     {
      glColor3fv (Blue);  obdual2[1].draw(); 
     }
   }
  if (DRAWDUALCTRLPOLY)
   {
    glColor3fv (Blue);  
    for (i=0; i<n; i++)  obdual2[i].drawCtrlPoly(); 
   }
  if (DRAWTANG)	  // draw pt associated with tangent, on first dual at tActive
   {
    glColor3fv (Red);  obdual2[0].drawPt(tActive);
   }
...
//      obdualselfreflex[i].prepareDisplay (nPtsPerSegment*30);
    ...
    for (i=0; i<n; i++)
     {
//    obdual2[i].dualOf2 (obstacle[i]);
      obdual2[i].oldDualOf (obstacle[i]);
...
if (1==1) {   
    ...
    tDelta = obstacle[0].getKnot (obstacle[0].getnKnot()-1) / 4000.; 	// 8000.
