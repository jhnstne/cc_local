...
#include "shape/Draw.h"                                      // DrawImplicitLine
...
static GLfloat   transx,transy,zoom, zoomduala, zoomdualb;
...
const int ZOOM=0;                                     // left-mouse mode toggles
static GLboolean transformMode=ZOOM;                  // 0=ZOOM
...
int   objWin;	                             // primal window identifier
...
BezierCurve2fArr obj;	                    // interpolating cubic Bezier curves
Array<TangentialCurve> duala;	               // associated tangential a-curves
Array<TangentialCurve> dualb;	               // associated tangential b-curves
BezierCurve2f hodo0;  // hodograph of 1st obj, for interactive tangent display
...
void gfxinit()
{
  ...
  glPointSize (6.0);
  transx = transy = 0.0; zoom = 1.5; zoomduala = 1.5; zoomdualb = .75;
}
...
void RotateOb (void)
{
  if (spinCCW)
   {
    tActive += tDelta;
    if (tActive > obj[0].getLastKnot()) tActive = obj[0].knot[0];
   }
  else
   {
    tActive -= tDelta;
    if (tActive < obj[0].knot[0]) tActive = obj[0].getLastKnot();
   }
  glutPostRedisplay();
}
...
{
  if (DRAWDUALOFMOUSE)
    screen2world (x,y,xsize,ysize,2, zoom, transx, transy, xMouse, yMouse);
  ...
  if (DRAWTANG)
   {
    ...
    obj[0].drawTangent (tActive, hodo0, 1);
    obj[0].drawPt 	    (tActive);
   }
  if (DRAWDUAL)
   {
    if (GRAY) glColor3fv (Black); else glColor3fv (Blue);
    V3f ptDual;  duala[0].evalProj (tActive, ptDual);
    DrawImplicitLine (ptDual[2], ptDual[1], ptDual[0]);
   }
  if (DRAWFIELD)
   {
    glColor3fv (Black);
    float avgSegLength = obj[0].domainLength() / obj[0].L;
    float stepsize = avgSegLength / nPtsPerSegment;
    float last = obj[0].getLastKnot();
    for (float t=obj[0].knot[0]; t<last; t += stepsize)
      obj[0].drawTangent (t, hodo0, 1);
   }
  if (DRAWDUALTANG)                           // dual of tangent of C^*(activet)
   {
    if (GRAY) glColor3fv (Black); else glColor3fv (Blue); 
    V2f pt;   duala[0].eval     (tActive, pt);
    V2f tang; duala[0].evalTang (tActive, tang);
...
void displayDual ()
{
  ...
  if (DRAWCURVE)
    for (i=0; i<obj.getn(); i++)       // active segments of tangential a-curves
     {
      if (GRAY) glColor3fv (greyscale[i%7]); else glColor3fv (colour[i%7]);
      duala[i].drawT (nPtsPerSegment/2);
     }
  ...
  if (DRAWTANG) 
   { 
    if (GRAY) glColor3fv (Black); else glColor3fv (Red); 
    duala[0].drawPt(tActive); 
   }
  if (DRAWDUALTANG)
   {
    if (GRAY) glColor3fv (Black); else glColor3fv (Blue); 
    duala[0].drawTangent (tActive);
   }
  if (DRAWDUALACTIVEPT)	
   {
    glColor3fv (Black);
    V2f p; obj[0].eval (tActive, p);
    DrawImplicitLine (1,p[1],p[0]);
    duala[0].eval(tActive,p);		                          // label point
...
void displayDual2 ()
{
  ...
  if (DRAWCURVE)
    for (i=0; i<obj.getn(); i++)       // active segments of tangential b-curves
     {
      if (GRAY) glColor3fv (greyscale[i%7]); else glColor3fv (colour[i%7]);
      dualb[i].drawT (nPtsPerSegment/2);
     }
  if (DRAWALLDUAL)
    for (i=0; i<obj.getn(); i++)
     {
...
int main (int argc, char **argv)
{
  ...
  InputClosedCurves (infile, nPtsPerSegment, obj);

  cout << "a" << endl;

  duala.allocate (obj.getn()); dualb.allocate (obj.getn());
  for (int i=0; i<obj.getn(); i++)
    BuildTangentialCurve (obj[i], nPtsPerSegment/2, duala[i], dualb[i]);

  obj[0].hodograph (hodo0);                         // for spinning tangent
  tActive = obj[0].knot[0];	                   // start at beginning
  if (LAPTOP) tDelta = (obj[0].getLastKnot() - tActive) / 2000.;
  else        tDelta = (obj[0].getLastKnot() - tActive) / 8000.;
  ...
  objWin = glutCreateWindow (titlebar);
