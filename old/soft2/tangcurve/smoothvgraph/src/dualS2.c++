...
BezierCurve3f		scaledHodo0;	// scaled hodograph of 1st obstacle
BezierCurve3f		scaledHodo1;	// scaled hodograph of 2nd obstacle
...
FloatArr		tHit0, tHit0True; // param values of intersections on obdual[0]
FloatArr		tHit1, tHit1True; // param values of intersection on obdual[1]
...
//    for (i=0; i<nHit; i++)
    for (i=2; i<3; i++)
      obstacle[0].drawTangentCircle (tHit0[i], scaledHodo0);
   }  
  if (DRAWCOMMONTANG)
   {
    glColor3fv (Black);
    for (i=0; i<nHitTrue; i++)
      obstacle[0].drawTangentCircle (tHit0True[i], scaledHodo0);
   }  
  ...
  if (DRAWTANG)
   {
    ...
    obstacle[0].drawTangentCircle (tActive, scaledHodo0);
//    obstacle[0].drawTangent (tActive, scaledHodo0);
   }
...
/******************************************************************************	
	Compare normals of tangent circles at endpoints.
	If selfFlag, culling mistakes from self tangents of obstacle 0.
******************************************************************************/

void cullMistakes (int nHit, FloatArr &tHit0, FloatArr &tHit1,
		   int &nHitTrue, FloatArr &tHit0True, 
		   FloatArr &tHit1True, int selfFlag)
{
  nHitTrue = 0;  tHit0True.allocate(nHit);  tHit1True.allocate(nHit);
  for (int i=0; i<nHit; i++)
   {
    // compare normals of two tangent circles at either end
    V3f pt0;  obstacle[0].eval (tHit0[i], pt0);
    V3f pt1;  obstacle[selfFlag ? 0 : 1].eval (tHit1[i], pt1);
    V3f tang0; scaledHodo0.eval (tHit0[i], tang0);
    V3f tang1; if (selfFlag) scaledHodo0.eval (tHit1[i], tang1); 
    	       else 	     scaledHodo1.eval (tHit1[i], tang1);
    // compute normals from tangents
    V3f norm0, norm1; norm0.cross (pt0,tang0);  norm1.cross (pt1, tang1);
norm0.print();
norm1.print();
    float angle0;  angle0 = norm0.angle (norm1);
cout << "Angle: " << angle0 << endl;
    float radeps = .1;
    if (angle0 < radeps || angle0 > M_PI-radeps)
     {
  ...
  if (!JUSTVIEWING)			
   {
    ...
    obstacle[0].createScaledHodo (scaledHodo0);
    obstacle[1].createScaledHodo (scaledHodo1);
...
/* identical to dual.c++ from here to STAR below */
...
/**********************remove common tangent mistakes***********************************/ 
    ...
    cullMistakes (nHit, tHit0, tHit1, nHitTrue, tHit0True, tHit1True, 0);
...
//   }
  ...
  glutAddMenuEntry ("All common tangents", 3);
