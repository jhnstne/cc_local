/*
  File:          dynamicLocalBackUmbra.cpp
  Author:        J.K. Johnstone 
  Created:	 25 May 2004
  Last Modified: 1 June 2004
  Purpose:       Analyze the local back umbra cast by an object B,
                 including dynamically changing B.
  Sequence:	 4th in a sequence (interpolate, tangCurve, bitang, dynamicLocalBackUmbra)
  Input: 	 Option 1: 
                 Two closed curves 
                 (defined through 2d polygons, implicitly defining interpolating cubic 
		 Bezier curves).

		 Option 2:
		 1 closed curve and 1 open curve.
		 The open curve defines the spine of a skeletal curve, for dynamic purposes.

		 The first curve is the distinguished object A.
  History: 	 5/24/04: Created from umbra.cpp.
                 6/28/04: Allowed exchange of A and B (to grow A).
*/
...
static void usage()
 {
  cout << "Build the back umbra cast by a single object B in a smooth 2D scene." << endl;
  cout << "Usage is " << RoutineName << endl;
  cout << "\t[-a] second curve is the open spine of a skeleton curve" << endl;
  cout << "\t[-A] first  curve is the open spine of a skeleton curve" << endl;
  cout << "\t[-R envelope radius] (default .1)" << endl;
  cout << "\t[-D sampling step size] (default .1)" << endl;
  ...
  cout << "\t    3: self-inner bitangents" << endl;
...
static GLboolean DRAWSELFINNER=0;       // draw self-inner bitangents?
static GLboolean DRAWLATEALL=0;         // draw umbraOb-umbraExtreme segments of all outer
                                // and self-inner bitangents, which will define the umbra?
...
static GLboolean LABEL=0;		// label the light (with 0) and obstacles (with 1,2,...)?
static GLboolean LETTERLABEL=0;         // label the light (with L) and obstacles (A,B,C,...)?
static GLboolean LABELLIGHT=1;          // label the light with L?
...
static GLboolean DYNAMICSPINE=0;        // is 2nd input curve the open spine of a skeleton curve,
                                        // used for dynamic demos?
static GLboolean OTHERDYNAMICSPINE=0;   // is 1st input curve the open spine of a skeleton curve?
int              level=15;              // computation level

Array<BezierCurve2f> 	obstacle;	// interpolating cubic Bezier curves
int                     nOb=2;          // redundant, but useful
...
Array<UmbralBitangArr>  selfinner;      // self-inner bitangents for each non-light obstacle
...
void keyboard (unsigned char key, int x, int y)
{
  ...
  case 'c':	ANIMATEINPIERCESWEEP  = !ANIMATEINPIERCESWEEP;		
		...
		break;
  case 'd':     DRAWOBSTACLE        = !DRAWOBSTACLE;        break;
  ...
  case 'S':     DRAWSELFINNER       = !DRAWSELFINNER;       break;
...
void menuOb (int value)
{
  ...
  case 101:     tGrow += .1;                           // animate the envelope growth 
                if (tGrow > spine.getLastKnot())
		  tGrow = spine.getLastKnot();              break;
  case 103:     DRAWSELFINNER       = !DRAWSELFINNER;       break;
  case 104:     DRAWLATEALL         = !DRAWLATEALL;         break;
  default:   					            break;
  }
  glutPostRedisplay();
}
...
void inputCurves (char *file, Array<BezierCurve2f> &obstacle)
{
  ...
  for (i=0; i<Pt.getn(); i++)
   { 
    if      (i==1 && DYNAMICSPINE)      obstacle[1].fit (Pt[1]);
    else if (i==0 && OTHERDYNAMICSPINE) obstacle[0].fit (Pt[0]);
    else                                obstacle[i].fitClosed (Pt[i]);
    if      (i==1 && DYNAMICSPINE)      obstacle[1].prepareDisplay (nPtsPerSegment, 1);
    else if (i==0 && OTHERDYNAMICSPINE) obstacle[0].prepareDisplay (nPtsPerSegment, 1);
...
{  
  ...
  for (i=1; i<nOb; i++)				// each obstacle
   {
    outer[i].allocate(10); // arbitrarily assume 10 is the limit
	...
	{
	  //	  assert (npo < 2);  no longer true when B-outer, say of tsint.pts, 
	  //                         are considered
	  if (npo==10) 
	    { 
	      cout<<"Expand allocation to outer in buildOuterBitang" << endl; 
	      exit(-1); 
	    }
    ...
    //    assert (npo<=2); // when surrounding, can no longer insist on exactly two
    //                        no longer true at all when B-outer are considered
    if (npo < 10) { cout << "Shrinking" << endl; outer[i].shrink(npo); }
    // trim L-outer pair of obstacle[i] if they intersect
    V2f hit;
    if (npo==2 && outer[i][0].intersect (outer[i][1], hit, .001))
     {
...
/******************************************************************************
	Filter selfbitangents down to self-inner bitangents.
	Only compute for non-light obstacles.
******************************************************************************/

void buildSelfInnerBitang (Array<UmbralBitangArr> &selfbitang,
			   Array<BezierCurve2f>   &obstacle,
			   float epsIntersect, float featureSize, 
			   float sameSideEps,
			   Array<UmbralBitangArr> &selfinner)
{  
  cout << "Building self-inner bitangents" << endl;    
  int i,j;
  selfinner.allocate(nOb); 
  for (i=1; i<nOb; i++)			// each non-distinguished obstacle
   {
     selfinner[i].allocate(10);  // assume arbitrarily that there are no more than 10
     int npbi=0; V2f umbraOb, umbraExtreme; int ob; float tOb;
     for (j=0; j<selfbitang[i].getn(); j++)
      {
	cout << endl << "Selfbitangent " << j << " of " << selfbitang[i].getn()
	     << " from obstacle " << i << endl;
	if (selfbitang[i][j].selfinner(obstacle, epsIntersect, featureSize, sameSideEps, 
				       radiusRoom, umbraOb, umbraExtreme, ob, tOb))
	  {
	    if (npbi==10) { cout << "Too many selfinner bitangents: increase allocation in buildSelfInnerBitang" << endl; exit(-1); }
	    selfinner[i][npbi++].create (selfbitang[i][j], umbraOb, umbraExtreme, ob, tOb);
	  }
      }
   }
}
...
void defineBackUmbra (Array<UmbralBitangArr> &ut, Array<Polygon2f> &backUmbra, 
		      float epsIntersect, float sameSideEps)
{
  cout << endl << "Building local back umbra" << endl;
  int i,j;
  ...
  for (i=1; i<nOb; i++)     // each obstacle has a local umbra
   {
    ...
    V2f ptOnObstacle;  obstacle[i].eval((ut[i][0].tOb + ut[i][1].tOb) / 2., ptOnObstacle);
			...
			cout << "ptOnLight = " << ptOnLight << endl;
	         cout << "ptOnObstacle parameter = " << (ut[i][0].tOb + ut[i][1].tOb) / 2.
		      << endl;
	...
	L.create (room[iEdge], room[(iEdge+1)%room.getn()]);
                        ...
                        cout << "2nd umbraExtreme " << ut[i][1].umbraExtreme 
			     << " lies on edge "    << room[iEdge] 
...
void buildLocalBackUmbra()
{
  ...
  if (level >= 2)
    buildOuterBitang (bitang, obstacle, epsIntersect, closeEps, featureSize, sameSideEps, outer);
  if (level >= 3)
    buildSelfInnerBitang (selfbitang, obstacle, epsIntersect, 
			  featureSize, sameSideEps, selfinner);
  /*
  if (level >= 3)
    buildInnerBitang (bitang, obstacle, epsIntersect, closeEps, featureSize, sameSideEps, inner);
  if (level >= 4)
    buildOuterPierce (bitang, obstacle, epsIntersect, closeEps, sameSideEps, outerpierce);
  if (level >= 5)
    buildInnerPierce (bitang, obstacle, epsIntersect, closeEps, sameSideEps, innerpierce);
  */
  if (level >= 6)
  ...
  if (DYNAMICSPINE)
    {
      ...
      specialHodo.createHodograph (obstacle[1]);
      buildLocalBackUmbra();
    } 
  else if (OTHERDYNAMICSPINE)
    {
      spine.buildEnvelope (spine.getKnot(0), tGrow, delta, R, obstacle[0]);
      specialHodo.createHodograph (obstacle[0]);
      buildLocalBackUmbra();
    }
  else
    buildLocalBackUmbra ();
  ...
  if (DRAWOTANG)			// outer bitangents
   {
    for (i=1; ONLYFIRST ? i<2 : i<nOb; i++)
      {
	glColor3fv (color[3*i]);
	for (j=0; j<outer[i].getn(); j++)
	  outer[i][j].draw ();
      }
   }
  if (DRAWSELFINNER)                    // self-inner bitangents
    {
      for (i=1; ONLYFIRST ? i<2 : i<nOb; i++)
	{
	  glColor3fv (color[3*i]);
	  for (j=0; j<selfinner[i].getn(); j++)
	    selfinner[i][j].draw();
	}
    }
  if (DRAWLATEALL)                      // draw umbraOb->umbraExtreme segments 
                                        // of umbral bitangents
    {
      glColor3fv (Black);
      for (i=0; i<outer[1].getn(); i++)
	outer[1][i].drawObExtreme ();
      for (i=1; ONLYFIRST ? i<2 : i<nOb; i++)
	for (j=0; j<selfinner[i].getn(); j++)
	  selfinner[i][j].drawObExtreme();
    }
...
int main (int argc, char **argv)
{
  ...
  while (ArgsParsed < argc)
   {
    if ('-' == argv[ArgsParsed][0])
      ...
      {
      case 'a': DYNAMICSPINE=1;                                 break;
      case 'A': OTHERDYNAMICSPINE=1;                            break;
      case 'c': closeEps = atof(argv[ArgsParsed++]);            break;
      case 'd': nPtsPerSegment = atoi(argv[ArgsParsed++]);	break;
      case 'D': delta = atof(argv[ArgsParsed++]);               break;
  ...
  if (DYNAMICSPINE)           { spine = obstacle[1]; tGrow = spine.getKnot(0) + .2; }
  else if (OTHERDYNAMICSPINE) { spine = obstacle[0]; tGrow = spine.getKnot(0) + .2; }
  ...
  int xleft = 900;	// x-coord of lefthand side for large windows
  int xsize = 600, ysize = 600;		// large windows

  glutInitWindowPosition (xleft,titleht+200);		// primal window
  ...
  glutAddMenuEntry ("Grow the envelope [g]",                    101);
  glutAddMenuEntry ("Shrink the envelope [G]",                  102);
  glutAddMenuEntry ("Obstacles [o]",                            100);
  ...
  glutAddMenuEntry ("Late segments of outer and self-inner bitangents", 104);
  glutAddMenuEntry ("Intermediate segment of a bitangent",      29);  
  glutAddMenuEntry ("Outer bitangents with L [o]",      	2);
  glutAddMenuEntry ("Self-inner bitangents [S]",                103);
